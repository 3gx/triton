...
def tmem_reduction_kernel(
    ...
    global_memory_layout: ttgl.constexpr = ttgl.BlockedLayout(
        [1, 1], [1, 32], [1, num_warps], [1, 0]
    )
    global_memory_layout_1d: ttgl.constexpr = ttgl.BlockedLayout(
        [1], [32], [num_warps], [0]
    )
    ...
    output, reduced = tmem.load(
        tmem_reg_layout, red_op=RED_OP, abs=USE_ABS, NaN=USE_NAN
    )
...
def test_tmem_reduction(red_op, use_abs, use_nan, M, N, num_warps):
    ...
    torch.testing.assert_close(
        expected_red, red_output, atol=1e-5, rtol=1e-5, equal_nan=use_nan
    )
