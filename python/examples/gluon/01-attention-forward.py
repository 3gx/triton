...
@gluon.jit
def _subtiled_qk_load(config, s_tmem, has_tmem_red: gl.constexpr):
...
def _softmax_tile(tile_id: gl.constexpr, config, M, desc_o, STAGE: gl.constexpr,  #
                  s_chnl, corr_chnl, exp_turnstile, has_tmem_red: gl.constexpr):
    ...
    for pid in range(scheduler.start_pid, scheduler.num_tiles, config.NUM_SMS):
        ...
        if STAGE & 1:
            m_i, l_i, corr_bar, s_consumer, corr_producer, exp_turnstile = _softmax_inner_loop(  #
                ...
                offs_m, m_i, l_i, STAGE=4 - STAGE, has_tmem_red=has_tmem_red)
        if STAGE & 2:
            m_i, l_i, corr_bar, s_consumer, corr_producer, exp_turnstile = _softmax_inner_loop(  #
                tile_id, config, prog, s_consumer, corr_producer, exp_turnstile, corr_bar,  #
                offs_m, m_i, l_i, STAGE=2, has_tmem_red=has_tmem_red)
...
@gluon.jit
def _attn_fwd_softmax0(config, chnls, descs, M, STAGE: gl.constexpr, has_tmem_red: gl.constexpr):
    q_chnl, kv_chnl, o_chnl, epi_chnl, s0_chnl, s1_chnl, c0_chnl, c1_chnl, exp_turnstile = chnls
    desc_q, desc_k, desc_v, desc_o = descs
    _softmax_tile(0, config, M, desc_o, STAGE, s0_chnl, c0_chnl, exp_turnstile.create_producer(), has_tmem_red)


@gluon.jit
def _attn_fwd_softmax1(config, chnls, descs, M, STAGE: gl.constexpr, has_tmem_red: gl.constexpr):
    q_chnl, kv_chnl, o_chnl, epi_chnl, s0_chnl, s1_chnl, c0_chnl, c1_chnl, exp_turnstile = chnls
    desc_q, desc_k, desc_v, desc_o = descs
    _softmax_tile(1, config, M, desc_o, STAGE, s1_chnl, c1_chnl, exp_turnstile.create_consumer(), has_tmem_red)
...
def attention_kernel(  #
        ...
        num_warps: gl.constexpr, has_tmem_red: gl.constexpr):
    ...
    gl.warp_specialize([
        ...
        (_attn_fwd_softmax0, (config, chnls, descs, M, STAGE, has_tmem_red)),
        (_attn_fwd_softmax1, (config, chnls, descs, M, STAGE, has_tmem_red)),
...
def attention_forward(q, k, v, causal, sm_scale, has_tmem_red):
    ...
    attention_kernel[(grid, )](
        ...
        num_warps=4, maxnreg=128, has_tmem_red=has_tmem_red)
...
@pytest.mark.parametrize("has_tmem_red", [False, True])
@pytest.mark.skipif(not is_blackwell(), reason="Gluon attention is only supported on Blackwell GPUs")
def test_op(Z, H, N_CTX, HEAD_DIM, causal, dtype, has_tmem_red, profile=False):
    device = "cuda"

    if has_tmem_red and not is_blackwell_ultra():
    ...
    tri_out, _ = attention_forward(q, k, v, causal, sm_scale, has_tmem_red)
...
for Z, H, D, is_causal, has_tmem_red in itertools.product(BATCH, N_HEADS, HEAD_DIM, causal, has_tmem_reds):
    config = triton.testing.Benchmark(
        ...
        ylabel="TFLOPS",
        plot_name=f"Attention Z={Z} H={H} D={D} causal={is_causal} has_tmem_red={has_tmem_red}",
        args={
            "Z": Z,
            "H": H,
            "HEAD_DIM": D,
            "causal": is_causal,
            "has_tmem_red": has_tmem_red,
        },
    )
...
def bench(Z, H, N_CTX, HEAD_DIM, causal, has_tmem_red, provider):
    ...
    with torch.nn.attention.sdpa_kernel([torch.nn.attention.SDPBackend.CUDNN_ATTENTION]):
        if provider == "triton":
            fn = lambda: attention_forward(q, k, v, causal, sm_scale, has_tmem_red)
        elif provider == "cudnn":
