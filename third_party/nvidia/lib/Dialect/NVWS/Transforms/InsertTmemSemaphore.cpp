...
// ---------------------------------------------------------------------------
// Helpers reused verbatim from InsertTmemAref.cpp
// ---------------------------------------------------------------------------
...
// TmemAccessDag — reused verbatim from InsertTmemAref.cpp
...
// ---------------------------------------------------------------------------
// New: TMEMSemaphore struct (replaces TMEMAref)
// ---------------------------------------------------------------------------
...
struct TMEMSemaphore {
  ...
  void release(OpBuilder &b, Location loc) {
    ...
    // Cross-release: PUT releases full, GET releases empty
    Value sem = (kind == PUT) ? full : empty;
  ...
  Value getBuffer(OpBuilder &b, std::optional<PartitionId> pid,
    ...
    if (!buffer) {
      ...
      Type dataBufType = getArefViewBufferType(bufType);
      // Make mutable
...
// ---------------------------------------------------------------------------
// DAG traversal — adapted from insertTmemArefImpl
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Main insertion function — adapted from insertTmemAref
// ---------------------------------------------------------------------------
...
int insertTmemSemaphore(TmemAccessDag &accessDag, int numTmemBlocks) {
  ...
  auto depth = getArefDepth(semBufType);
  ...
  if (state.kind == TMEMSemaphore::GET) {
...
// ---------------------------------------------------------------------------
// Driver — adapted from InsertTmemAref runOnFunction
// ---------------------------------------------------------------------------
