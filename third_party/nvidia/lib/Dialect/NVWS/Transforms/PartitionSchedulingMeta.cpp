...
static void insertAll(SetVector<int> &dst, ArrayRef<int> src) {
  dst.insert(src.begin(), src.end());
}

namespace ttng = triton::nvidia_gpu;

#define DEBUG_TYPE "nvws-partition-scheduling-meta"
...
#define GEN_PASS_DEF_NVWSPARTITIONSCHEDULINGMETA
#include "nvidia/include/Dialect/NVWS/Transforms/Passes.h.inc"
} // namespace mlir

namespace {
struct NVWSPartitionSchedulingMeta
    : public mlir::triton::impl::NVWSPartitionSchedulingMetaBase<
          NVWSPartitionSchedulingMeta> {
  using NVWSPartitionSchedulingMetaBase::NVWSPartitionSchedulingMetaBase;

  void runOnOperation() override;
  LogicalResult runOnFuncOp(FuncOp funcOp);
};
} // namespace
...
void NVWSPartitionSchedulingMeta::runOnOperation() {
  FuncOp funcOp = getOperation();
  Operation *cloneOp = funcOp->clone();
  auto cloneFunc = cast<FuncOp>(cloneOp);

  if (failed(runOnFuncOp(cloneFunc))) {
    cloneOp->destroy();
    signalPassFailure();
    return;
  }

  funcOp->setAttrs(cloneOp->getAttrs());
  funcOp.getBody().takeBody(cloneFunc.getBody());
  cloneOp->destroy();
}
