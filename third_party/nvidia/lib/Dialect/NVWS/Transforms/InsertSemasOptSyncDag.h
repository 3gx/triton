...
buildEmitterTransitionPlan(const OptSyncDag &dag, const SyncPlan &sp,
  ...
  auto addSourceReleases = [&](unsigned groupIdx, ArrayRef<unsigned> edgeIdxs) {
    llvm::MapVector<Region *, SmallVector<unsigned, 4>> edgesByEnter;
  ...
  auto addSiteRelease = [&](unsigned groupIdx, unsigned edgeIdx) {
    addSourceReleases(groupIdx, ArrayRef<unsigned>(&edgeIdx, 1));
  };
