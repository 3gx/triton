...
current solid-01 pass. For lit-backed cases, the edges, acquire sites, release
...
source and their live solid-01 dumps, but are not claimed to be current lit
contracts.
...
`@container_with_disjoint_subviews` is the current solid-01 replacement for
the conceptual disjoint-piece example. One container covers three pairwise
disjoint subviews:
...
In pure ordering terms, `c3` followed by `c5` would imply `c4`. Solid-01
...
loop has a semaphore-token `iter_arg` in the current solid-01 IR. The outer
...
was generated by running the shown input through the current solid-01 pass;
...
from running this shape through the current solid-01 pass:
...
asynchronous completion and the edge is marked `preserve`. Solid-01 therefore
...
current solid-01 implementation does not relocate an acquire. DirectBuilder
...
Solid-01 emits no semaphore-token result from the `if`.
