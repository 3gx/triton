...
[NVWS-AWS terminology](nvws-aws-overview.md#terminology) — is the planner's
input record: it bounds the allocation's lifetime and is never read by
`InsertSemas`.
...
The [NVWS-AWS terminology](nvws-aws-overview.md#terminology) defines the
terms:
...
Allocations that reuse an existing `buffer.id` are stacked within their
owner's columns: the TMEM pre-assign path never advances the column offset
(`nextColOffset += 0` in `MemoryPlanner.cpp`, matching the Meta baseline).
Every planner path therefore keeps a `buffer.id` group's members
overlapping into one connected component — the invariant `InsertSemas`
asserts before analyzing the group (see
[ACCESS-DAG](insert-semas/access-dag.md#pieces-must-connect)).
