...
// threads one acquire token through both loops (bottom re-acquire carried via
// yield); buffer.id=4 stays point-of-use behind its own in-loop gate. The
...
// ASP-LABEL:     tt.func public @_attn_fwd_persist(
  tt.func public @_attn_fwd_persist(%sm_scale: f32, %M: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %Z: i32, %H: i32 {tt.divisibility = 16 : i32}, %desc_q: !tt.ptr<f16> {tt.divisibility = 16 : i32}, %desc_k: !tt.ptr<f16> {tt.divisibility = 16 : i32}, %desc_v: !tt.ptr<f16> {tt.divisibility = 16 : i32}, %desc_o: !tt.ptr<f16> {tt.divisibility = 16 : i32}) attributes {noinline = false} {
    ...
    // multi-member group: one true gate (pending_count = 2) plus six
    ...
    // per-iteration accumulator tokens; R5/ACC0/ACC1 thread through the outer
    // loop.
    ...
    // tokens. R4 stays point-of-use behind its own gates (no carried token).
    ...
    %tile_idx = scf.for %_ = %c0_i32 to %2 step %c1_i32 iter_args(%tile_idx_19 = %prog_id) -> (i32)  : i32 {
      ...
      // Inner pipelined loop re-threads only the R5 token. The R4 inner gate
      ...
      %offsetkv_y_40:9 = scf.for %offsetkv_y_88 = %c0_i32 to %c16384_i32 step %c128_i32 iter_args(%offset_y_89 = %offset_y_22, %arg12 = %cst_2, %arg13 = %cst_1, %qk_0_90 = %qk_0_32, %acc_91 = %acc_0_34, %arg16 = %cst_2, %arg17 = %cst_1, %qk_1_92 = %qk_1_33, %acc_93 = %acc_1_35) -> (i32, tensor<128xf32, #ttg.slice<{dim = 1, parent = #linear}>>, tensor<128xf32, #ttg.slice<{dim = 1, parent = #linear}>>, !ttg.async.token, !ttg.async.token, tensor<128xf32, #ttg.slice<{dim = 1, parent = #linear}>>, tensor<128xf32, #ttg.slice<{dim = 1, parent = #linear}>>, !ttg.async.token, !ttg.async.token)  : i32 {
        ...
        // End-of-iteration re-acquire for the R5 carrier, threaded into the
        // next inner iteration via scf.yield.
      ...
      // R5's carried result and R4's point-of-use drain each open one final
      // FULL phase.
      ...
      // Both post-loop stats stores (partition 4) land in the single R5_F5
      ...
      // buffer #1, then releases R5_F6 and the first R5_E arrive.
      ...
      // Outer-loop end bridges R4 back to its point-of-use gate (two arrives)
      // and re-acquires the R5 carrier for the next persistent iteration; the
      // ACC0/ACC1 readout tokens ride the yield.
      ...
      // CHECK:           scf.yield {ttg.partition = array<i32: 0, 1, 2, 3, 4, 5>} %{{.*}}, [[OX_R5]], [[ACCF0_AE]], [[ACCF1_AE]] : i32, !ttg.async.token, !ttg.async.token, !ttg.async.token
