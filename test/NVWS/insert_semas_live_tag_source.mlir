...
// A TMEM accumulator whose live tag source (the alloc token) is threaded
// through the outer loop, with a prior inner loop between the accumulator
// protocol and a later SMEM scratch protocol. Exercises point-of-use
// lowering: the inner loop threads no token ([]), the MMA partition acquires
// at its first touching access inside the body, and a post-loop bridge
// acquire anchors the release back to the accumulator EMPTY semaphore.
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @live_tag_source_after_prior_loop_threading(%lb: i32, %ub: i32, %step: i32) {
    ...
    // Single-buffered (1x): SMEM scratch EMPTY/FULL pair, then TMEM
    // accumulator EMPTY sem plus store/load->MMA and MMA->load handoff sems.
    // The initial EMPTY acquire is threaded into the outer loop.
    ...
    %outer = scf.for %iv0 = %lb to %ub step %step iter_args(%tile = %c0_i32) -> (i32) : i32 {
      // Init store into the EMPTY buffer, then hand the buffer off to the MMA
      // partition.
      ...
      // Point-of-use: no token is threaded through the inner loop.
      ...
      %inner_tmem = scf.for %iv1 = %lb to %ub step %step iter_args(%tok1 = %tok) -> (!ttg.async.token) : i32 {
        ...
        // The MMA partition acquires at its first touching access inside the
        // loop body (point-of-use), not before the loop.
        ...
        // CHECK: [[V14:%.*]] = nvws.semaphore.buffer [[V6]], [[V13]] {ttg.partition = array<i32: 1>} : <[!ttg.memdesc<1x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 1x128x128>
        ...
        // MMA -> load handoff; the load then hands [[V6]] back for the next MMA.
        // CHECK: nvws.semaphore.release [[V7]], [[V13]] [#nvws.async_op<tc5mma>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>} : <[!ttg.memdesc<1x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>, !ttg.async.token
        // CHECK: [[V15:%.*]] = nvws.semaphore.acquire [[V7]] {ttg.partition = array<i32: 0>} : <[!ttg.memdesc<1x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]> -> !ttg.async.token
        // CHECK: [[V16:%.*]] = nvws.semaphore.buffer [[V7]], [[V15]] {ttg.partition = array<i32: 0>} : <[!ttg.memdesc<1x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 1x128x128>
        ...
        // CHECK: nvws.semaphore.release [[V6]], [[V15]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 0>} : <[!ttg.memdesc<1x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>, !ttg.async.token
      ...
      // Post-loop bridge: partition 1 re-acquires [[V6]] to anchor the
      // release of [[V5]] at the last MMA (release stays after the loop);
      // then partition 0 acquires [[V5]] for the final load.
      // CHECK: [[V17:%.*]] = nvws.semaphore.acquire [[V6]] {ttg.partition = array<i32: 1>} : <[!ttg.memdesc<1x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]> -> !ttg.async.token
      ...
      // Scratch producer: acquire EMPTY, store, release FULL.
      ...
      // Scratch consumer: acquire FULL once before its loop (its stages
      // match), buffer inside, release EMPTY after the loop.
      ...
      // The outer loop carries the accumulator EMPTY token to the next tile.
    ...
    // CHECK: } {tt.warp_specialize, ttg.partition = array<i32: 0, 1, 2>, ttg.partition.outputs = [array<i32: 0, 1, 2>, array<i32: 0>], ttg.warp_specialize.tag = 0 : i32}
