...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @tma_gather(%desc: !tt.tensordesc<1x128xf16, #shared_desc>) {
    ...
    // CHECK-COUNT-3: ttng.init_barrier %{{.*}}, 1 : !ttg.memdesc<1xi64, #shared, #smem, mutable>
    %sem = nvws.semaphore.create %buf {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x8x128xf16, #shared_desc, #smem, mutable>]>
  ...
  tt.func @tc5mma_commit() {
    ...
    %sem = nvws.semaphore.create %buf {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>
  ...
  tt.func @wgmma_pending_count() {
    ...
    %sem = nvws.semaphore.create %buf released = 1 {pending_count = 2 : i32} : !nvws.semaphore<[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>
  ...
  tt.func @fence_needed() {
    ...
    %sem_generic = nvws.semaphore.create %buf released = 1 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>
    ...
    %sem_other = nvws.semaphore.create %buf {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>
  ...
  tt.func @tmem_scales_passthrough(%arg0: !ttg.memdesc<128x8xi8, #tmem_scales, #ttng.tensor_memory>) {
    ...
    %cm1_i32 = arith.constant -1 : i32
    %c1_i32 = arith.constant 1 : i32
    %sem = nvws.semaphore.create %arg0 released = 1 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<128x8xi8, #tmem_scales, #ttng.tensor_memory>]>
  ...
  tt.func @two_consumers(%arg0: i32, %arg1: i32, %arg2: i32) {
    ...
    %sem_empty = nvws.semaphore.create %buf released = 7 {pending_count = 2 : i32} : !nvws.semaphore<[!ttg.memdesc<3x1xi32, #shared, #smem, mutable>]>
    ...
    %sem_full = nvws.semaphore.create %buf {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<3x1xi32, #shared, #smem, mutable>]>
    // CHECK: scf.for
  ...
  tt.func @three_consumers(%arg0: i32, %arg1: i32, %arg2: i32) {
    ...
    %sem_empty = nvws.semaphore.create %buf released = 7 {pending_count = 3 : i32} : !nvws.semaphore<[!ttg.memdesc<3x1xi32, #shared, #smem, mutable>]>
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @reuse_argument(%arg0: i32, %arg1: i32, %arg2: i32) {
    ...
    %empty = nvws.semaphore.create %alloc released = 1 {pending_count = 2 : i32} : !nvws.semaphore<[!ttg.memdesc<1x1xi32, #shared4, #smem4, mutable>]>
    %full = nvws.semaphore.create %alloc {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x1xi32, #shared4, #smem4, mutable>]>
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @warp_specialize_tma_matmul(%desc_a: !tt.tensordesc<128x64xf16, #shared_ws_port>, %desc_b: !tt.tensordesc<128x64xf16, #shared_ws_port>, %lb: i32, %ub: i32, %step: i32) {
    ...
    // At depth one, mask 1 is all-released. Multibuffering must replicate that
    // state across every newly materialized stage.
    %empty_a = nvws.semaphore.create %alloc_a released = 1 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared_ws_port, #smem_ws_port, mutable>]>
    %full_a = nvws.semaphore.create %alloc_a {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared_ws_port, #smem_ws_port, mutable>]>
    %empty_b = nvws.semaphore.create %alloc_b released = 1 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared_ws_port, #smem_ws_port, mutable>]>
    %full_b = nvws.semaphore.create %alloc_b {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared_ws_port, #smem_ws_port, mutable>]>
    // The two semaphore pairs are combined into one EMPTY/FULL pair.
    ...
    %0 = scf.for %iv = %lb to %ub step %step iter_args(%acc = %token) -> (!ttg.async.token) : i32 {
      // CHECK: [[TMA_EMPTY_WAIT:%.*]] = ttg.memdesc_index [[TMA_EMPTY]][%{{.*}}] {ttg.partition = array<i32: 0>}
      // CHECK: ttng.wait_barrier [[TMA_EMPTY_WAIT]], %{{.*}} {ttg.partition = array<i32: 0>}
      // CHECK: [[BUF_B_SLICE:%.*]] = ttg.memdesc_index [[BUF_B]][%{{.*}}] {ttg.partition = array<i32: 0>}
      // CHECK: [[BUF_A_SLICE:%.*]] = ttg.memdesc_index [[BUF_A]][%{{.*}}] {ttg.partition = array<i32: 0>}
      // CHECK: [[TMA_FULL_SLICE:%.*]] = ttg.memdesc_index [[TMA_FULL]][%{{.*}}] {ttg.partition = array<i32: 0>}
      ...
      // CHECK: [[TMA_FULL_WAIT:%.*]] = ttg.memdesc_index [[TMA_FULL]][%{{.*}}] {ttg.partition = array<i32: 1>}
      // CHECK: ttng.wait_barrier [[TMA_FULL_WAIT]], %{{.*}} {ttg.partition = array<i32: 1>}
      // CHECK: [[BUF_B_SLICE_C:%.*]] = ttg.memdesc_index [[BUF_B]][%{{.*}}] {ttg.partition = array<i32: 1>}
      // CHECK: [[BUF_A_SLICE_C:%.*]] = ttg.memdesc_index [[BUF_A]][%{{.*}}] {ttg.partition = array<i32: 1>}
      ...
      // CHECK: [[TMA_EMPTY_COMMIT:%.*]] = ttg.memdesc_index [[TMA_EMPTY]][%{{.*}}] {ttg.partition = array<i32: 1>}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @load_used_as_reg_and_smem(%arg0: !tt.tensordesc<128x64xf16, #shared_mixed>, %arg1: i32) {
    ...
    %empty = nvws.semaphore.create %buf released = 1 {pending_count = 2 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared_mixed, #smem_mixed, mutable>]>
    %full = nvws.semaphore.create %buf {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared_mixed, #smem_mixed, mutable>]>
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @load_used_as_reg_and_smem_same_partition(%arg0: !tt.tensordesc<128x64xf16, #shared_mixed>, %arg1: i32) {
    ...
    %buf = ttg.local_alloc : () -> !ttg.memdesc<1x128x64xf16, #shared_mixed, #smem_mixed, mutable>
    %empty = nvws.semaphore.create %buf released = 1 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared_mixed, #smem_mixed, mutable>]>
    %full = nvws.semaphore.create %buf {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared_mixed, #smem_mixed, mutable>]>
    scf.for %arg2 = %c0_i32 to %arg1 step %c1_i32 : i32 {
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @semaphore_buffer(%arg0: !tt.tensordesc<128x64xf16, #shared7>, %arg1: !tt.tensordesc<64x128xf16, #shared7>) {
    ...
    %empty = nvws.semaphore.create %result released = 3 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<2x128x128xf32, #tmem7, #ttng.tensor_memory, mutable>]>
    %full = nvws.semaphore.create %result {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<2x128x128xf32, #tmem7, #ttng.tensor_memory, mutable>]>
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @semaphore_not_in_loop(%arg0: i32, %arg1: i32, %arg2: i32, %arg3: !tt.tensordesc<128x64xf16, #shared5>, %arg4: !tt.tensordesc<128x64xf16, #shared5>) {
    ...
    // CHECK: [[MBAR_F:%.*]] = ttg.local_alloc : () -> !ttg.memdesc<1x1xi64,
    // CHECK: ttng.init_barrier {{.*}}, 1
    %empty = nvws.semaphore.create %result released = 1 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x128xf32, #tmem5, #ttng.tensor_memory, mutable>]>
    %full = nvws.semaphore.create %result {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x128xf32, #tmem5, #ttng.tensor_memory, mutable>]>
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @semaphore_scale_mma_user(%arg0: !ttg.memdesc<128x64xf16, #shared9, #smem9>, %arg1: !ttg.memdesc<64x128xf16, #shared9, #smem9>, %arg2: !tt.tensordesc<8x128xi8, #shared9>, %arg3: !ttg.memdesc<128x8xi8, #tmem_scales9, #ttng.tensor_memory>, %arg4: i32) {
    ...
    %empty = nvws.semaphore.create %result released = 1 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x128xf32, #tmem9, #ttng.tensor_memory, mutable>]>
    %full = nvws.semaphore.create %result {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x128xf32, #tmem9, #ttng.tensor_memory, mutable>]>
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func public @attention_forward(%arg0: !ttg.memdesc<256x64xf16, #shared_attn, #smem_attn>, %arg1: !tt.tensordesc<64x64xf16, #shared_attn>, %arg2: f32, %arg3: i32) {
    ...
    %acc_empty = nvws.semaphore.create %acc released = 1 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x256x64xf32, #tmem_attn, #ttng.tensor_memory, mutable>]>
    %acc_full = nvws.semaphore.create %acc {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x256x64xf32, #tmem_attn, #ttng.tensor_memory, mutable>]>
    %tmp = ttng.tmem_alloc : () -> !ttg.memdesc<1x256x64xf32, #tmem_attn, #ttng.tensor_memory, mutable>
    %tmp_empty = nvws.semaphore.create %tmp released = 1 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x256x64xf32, #tmem_attn, #ttng.tensor_memory, mutable>]>
    %tmp_full = nvws.semaphore.create %tmp {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x256x64xf32, #tmem_attn, #ttng.tensor_memory, mutable>]>
    %kbuf = ttg.local_alloc : () -> !ttg.memdesc<1x64x64xf16, #shared_attn, #smem_attn, mutable>
    %k_empty = nvws.semaphore.create %kbuf released = 1 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x64x64xf16, #shared_attn, #smem_attn, mutable>]>
    %k_full = nvws.semaphore.create %kbuf {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x64x64xf16, #shared_attn, #smem_attn, mutable>]>
    %prob = ttg.local_alloc : () -> !ttg.memdesc<1x256x64xf16, #shared_attn, #smem_attn, mutable>
    %prob_empty = nvws.semaphore.create %prob released = 1 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x256x64xf16, #shared_attn, #smem_attn, mutable>]>
    %prob_full = nvws.semaphore.create %prob {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x256x64xf16, #shared_attn, #smem_attn, mutable>]>
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @multi_buffer_trans_chain() {
    ...
    %sem = nvws.semaphore.create %buf_a, %buf_b released = 3 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x64xf16, #shared, #smem, mutable>]>
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @ws_multibuffer_trans_chain(%desc_a: !tt.tensordesc<128x64xf16, #shared_ws>, %desc_b: !tt.tensordesc<128x64xf16, #shared_ws>, %lb: i32, %ub: i32, %step: i32) {
    ...
    %empty_a = nvws.semaphore.create %alloc_a released = 3 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared_ws, #smem_ws, mutable>]>
    %full_a = nvws.semaphore.create %alloc_a {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared_ws, #smem_ws, mutable>]>
    %empty_b = nvws.semaphore.create %alloc_b released = 3 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared_ws, #smem_ws, mutable>]>
    %full_b = nvws.semaphore.create %alloc_b {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared_ws, #smem_ws, mutable>]>
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @phase2_multibuffer(%desc: !tt.tensordesc<128x64xf16, #shared_desc2>, %lb: i32, %ub: i32, %step: i32) {
    ...
    %sem_empty = nvws.semaphore.create %buf released = 1 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared_desc2, #smem2, mutable>]>
    %sem_full = nvws.semaphore.create %buf {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared_desc2, #smem2, mutable>]>
...
// tc_gen5_mma consumer are combined into one pair. Verifies: 2 mbar arrays
// (not 4), barrier_expect 32768 (16384+16384), both async_tma_copy operations
// use the same mbar, and the consumer emits one tc_gen5_commit.
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @combine_two_tma_loads(%desc_a: !tt.tensordesc<128x64xf16, #shared3>, %desc_b: !tt.tensordesc<128x64xf16, #shared3>, %lb: i32, %ub: i32, %step: i32) {
    ...
    // Combined: only two mbar arrays (EMPTY + FULL), with three slots each.
    // CHECK: [[MBAR_E:%.*]] = ttg.local_alloc : () -> !ttg.memdesc<3x1xi64,
    // CHECK: ttng.init_barrier {{.*}}, 1
    // CHECK: ttng.init_barrier {{.*}}, 1
    // CHECK: ttng.init_barrier {{.*}}, 1
    ...
    // CHECK: ttng.init_barrier {{.*}}, 1
    // CHECK: ttng.init_barrier {{.*}}, 1
    %empty_a = nvws.semaphore.create %alloc_a released = 1 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]>
    %full_a = nvws.semaphore.create %alloc_a {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]>
    %empty_b = nvws.semaphore.create %alloc_b released = 1 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]>
    %full_b = nvws.semaphore.create %alloc_b {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]>
    // One shared stage, producer phase, and consumer phase.
    // CHECK: scf.for {{.*}} iter_args({{.*}}) -> (!ttg.async.token, i32, i32, i32)
    %0 = scf.for %iv = %lb to %ub step %step iter_args(%acc = %token) -> (!ttg.async.token) : i32 {
      // Producer: wait EMPTY, get both buffer views, then issue both TMA loads
      // against the combined FULL mbar with their summed transaction bytes.
      // CHECK: ttng.wait_barrier {{.*}} {ttg.partition = array<i32: 0>}
      ...
      // The combined EMPTY semaphore requires one consumer commit.
    ...
    // Cleanup: three invalidations and one deallocation per combined mbar.
    // CHECK: ttng.inval_barrier
    // CHECK: ttng.inval_barrier
    // CHECK: ttng.inval_barrier
    // CHECK: ttg.local_dealloc [[MBAR_E]]
  ...
  // CHECK-LABEL: @combine_mixed_completion_kinds
  tt.func @combine_mixed_completion_kinds(%desc_a: !tt.tensordesc<128x64xf16, #shared3>, %src_b: tensor<128x64xf16, #blocked3>, %lb: i32, %ub: i32, %step: i32) {
    %true = arith.constant true
    %alloc_a = ttg.local_alloc : () -> !ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>
    %alloc_b = ttg.local_alloc : () -> !ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>
    %acc, %token = ttng.tmem_alloc : () -> (!ttg.memdesc<128x128xf32, #tmem3, #ttng.tensor_memory, mutable>, !ttg.async.token)
    // Each input semaphore has one completion source and correctly authors 1.
    %empty_a = nvws.semaphore.create %alloc_a released = 1 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]>
    %full_a = nvws.semaphore.create %alloc_a {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]>
    %empty_b = nvws.semaphore.create %alloc_b released = 1 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]>
    %full_b = nvws.semaphore.create %alloc_b {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]>
    // Consumer tc5mma kinds deduplicate to EMPTY count 1.
    // CHECK: [[MIXED_E:%.*]] = ttg.local_alloc : () -> !ttg.memdesc<3x1xi64,
    // CHECK-COUNT-3: ttng.init_barrier {{.*}}, 1
    // Producer tma_load + none kinds union to FULL count 2.
    // CHECK: [[MIXED_F:%.*]] = ttg.local_alloc : () -> !ttg.memdesc<3x1xi64,
    // CHECK-COUNT-3: ttng.init_barrier {{.*}}, 2
    %result = scf.for %iv = %lb to %ub step %step iter_args(%iter = %token) -> (!ttg.async.token) : i32 {
      %pa = nvws.semaphore.acquire %empty_a {ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]> -> !ttg.async.token
      %pba = nvws.semaphore.buffer %empty_a, %pa {ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x64xf16, #shared3, #smem3, mutable>
      nvws.descriptor_load %desc_a[%iv, %iv] 16384 %pba {ttg.partition = array<i32: 0>} : !tt.tensordesc<128x64xf16, #shared3>, i32, i32, !ttg.memdesc<128x64xf16, #shared3, #smem3, mutable>
      nvws.semaphore.release %full_a, %pa [#nvws.async_op<tma_load>] {arrive_count = 1 : i32, ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]>, !ttg.async.token
      %pb = nvws.semaphore.acquire %empty_b {ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]> -> !ttg.async.token
      %pbb = nvws.semaphore.buffer %empty_b, %pb {ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x64xf16, #shared3, #smem3, mutable>
      ttg.local_store %src_b, %pbb {ttg.partition = array<i32: 0>} : tensor<128x64xf16, #blocked3> -> !ttg.memdesc<128x64xf16, #shared3, #smem3, mutable>
      nvws.semaphore.release %full_b, %pb [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]>, !ttg.async.token
      // CHECK: ttng.barrier_expect {{.*}}, 16384 {ttg.partition = array<i32: 0>}
      // CHECK: ttng.async_tma_copy_global_to_local
      // CHECK: ttg.local_store
      // CHECK: ttng.arrive_barrier {{.*}}, 1 {ttg.partition = array<i32: 0>}
      %ca = nvws.semaphore.acquire %full_a {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]> -> !ttg.async.token
      %cba = nvws.semaphore.buffer %full_a, %ca {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x64xf16, #shared3, #smem3, mutable>
      %cb = nvws.semaphore.acquire %full_b {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]> -> !ttg.async.token
      %cbb = nvws.semaphore.buffer %full_b, %cb {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x64xf16, #shared3, #smem3, mutable>
      %rhs = ttg.memdesc_trans %cbb {order = array<i32: 1, 0>, ttg.partition = array<i32: 1>} : !ttg.memdesc<128x64xf16, #shared3, #smem3, mutable> -> !ttg.memdesc<64x128xf16, #shared4, #smem3, mutable>
      %mma = ttng.tc_gen5_mma %cba, %rhs, %acc[%iter], %true, %true {ttg.partition = array<i32: 1>} : !ttg.memdesc<128x64xf16, #shared3, #smem3, mutable>, !ttg.memdesc<64x128xf16, #shared4, #smem3, mutable>, !ttg.memdesc<128x128xf32, #tmem3, #ttng.tensor_memory, mutable>
      nvws.semaphore.release %empty_a, %ca [#nvws.async_op<tc5mma>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]>, !ttg.async.token
      nvws.semaphore.release %empty_b, %cb [#nvws.async_op<tc5mma>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>]>, !ttg.async.token
      // CHECK: ttng.tc_gen5_commit {{.*}} {ttg.partition = array<i32: 1>}
      scf.yield %mma : !ttg.async.token
  ...
  tt.func @do_not_combine_mixed_depth_tma_loads(%desc_a: !tt.tensordesc<128x64xf16, #shared3>, %desc_b: !tt.tensordesc<128x64xf16, #shared3>, %lb: i32, %ub: i32, %step: i32) {
    ...
    // CHECK-DAG: ttg.local_alloc : () -> !ttg.memdesc<2x1xi64,
    // CHECK-COUNT-2: ttng.barrier_expect {{.*}}, 16384 {ttg.partition = array<i32: 0>}
    // CHECK-COUNT-2: ttng.wait_barrier {{.*}} {ttg.partition = array<i32: 1>}
    // CHECK-COUNT-2: ttng.tc_gen5_commit {{.*}} {ttg.partition = array<i32: 1>}
    %alloc_a = ttg.local_alloc {buffer.copy = 3 : i32} : () -> !ttg.memdesc<3x128x64xf16, #shared3, #smem3, mutable>
    %alloc_b = ttg.local_alloc {buffer.copy = 2 : i32} : () -> !ttg.memdesc<2x128x64xf16, #shared3, #smem3, mutable>
    %result, %token = ttng.tmem_alloc : () -> (!ttg.memdesc<128x128xf32, #tmem3, #ttng.tensor_memory, mutable>, !ttg.async.token)
    %empty_a = nvws.semaphore.create %alloc_a released = 7 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<3x128x64xf16, #shared3, #smem3, mutable>]>
    %full_a = nvws.semaphore.create %alloc_a {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<3x128x64xf16, #shared3, #smem3, mutable>]>
    %empty_b = nvws.semaphore.create %alloc_b released = 3 {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared3, #smem3, mutable>]>
    %full_b = nvws.semaphore.create %alloc_b {pending_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared3, #smem3, mutable>]>
    %0 = scf.for %iv = %lb to %ub step %step iter_args(%acc = %token) -> (!ttg.async.token) : i32 {
      ...
      %tok_cb = nvws.semaphore.acquire %full_b {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared3, #smem3, mutable>]> -> !ttg.async.token
      %cbuf_b = nvws.semaphore.buffer %full_b, %tok_cb {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared3, #smem3, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x64xf16, #shared3, #smem3, mutable>
      %rhs = ttg.memdesc_trans %cbuf_b {order = array<i32: 1, 0>, ttg.partition = array<i32: 1>} : !ttg.memdesc<128x64xf16, #shared3, #smem3, mutable> -> !ttg.memdesc<64x128xf16, #shared4, #smem3, mutable>
      %mma = ttng.tc_gen5_mma %cbuf_a, %rhs, %result[%acc], %true, %true {ttg.partition = array<i32: 1>} : !ttg.memdesc<128x64xf16, #shared3, #smem3, mutable>, !ttg.memdesc<64x128xf16, #shared4, #smem3, mutable>, !ttg.memdesc<128x128xf32, #tmem3, #ttng.tensor_memory, mutable>
      nvws.semaphore.release %empty_a, %tok_ca [#nvws.async_op<tc5mma>] {ttg.partition = array<i32: 1>, arrive_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<3x128x64xf16, #shared3, #smem3, mutable>]>, !ttg.async.token
      nvws.semaphore.release %empty_b, %tok_cb [#nvws.async_op<tc5mma>] {ttg.partition = array<i32: 1>, arrive_count = 1 : i32} : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared3, #smem3, mutable>]>, !ttg.async.token
      scf.yield {ttg.partition = array<i32: 0, 1>} %mma : !ttg.async.token
    } {ttg.partition = array<i32: 0, 1>, ttg.partition.outputs = [array<i32: 1>], tt.warp_specialize}
    ttg.local_dealloc %alloc_a : !ttg.memdesc<3x128x64xf16, #shared3, #smem3, mutable>
    ttg.local_dealloc %alloc_b : !ttg.memdesc<2x128x64xf16, #shared3, #smem3, mutable>
    tt.return
  }
}
