...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @multi_buffer_trans_chain() {
    ...
    %sem = nvws.semaphore.create %buf_a, %buf_b true : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x64xf16, #shared, #smem, mutable>], 2>
    %tok = nvws.semaphore.acquire %sem[%c0_i32, %cm1_i32] {loop.cluster = 6 : i32, loop.stage = 2 : i32, ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x64xf16, #shared, #smem, mutable>], 2> -> !ttg.async.token
    %views:2 = nvws.semaphore.buffer %sem[%c0_i32], %tok {loop.cluster = 6 : i32, loop.stage = 2 : i32, ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x64xf16, #shared, #smem, mutable>], 2>, !ttg.async.token -> !ttg.memdesc<128x64xf16, #shared, #smem, mutable, 1x128x64>, !ttg.memdesc<128x64xf16, #shared, #smem, mutable, 1x128x64>
    ...
    // CHECK-COUNT-2: ttg.memdesc_index %{{.*}}[%c0_i32] {loop.cluster = 6 : i32, loop.stage = 2 : i32, ttg.partition = array<i32: 1>} : !ttg.memdesc<2x128x64xf16, #shared, #smem, mutable> -> !ttg.memdesc<128x64xf16, #shared, #smem, mutable>
    ...
    // CHECK: "use_pair"([[A_TRANS]], %{{.*}}) : (!ttg.memdesc<64x128xf16, #shared2, #smem, mutable>, !ttg.memdesc<128x64xf16, #shared, #smem, mutable>) -> ()
    "use_pair"(%t, %views#1) : (!ttg.memdesc<64x128xf16, #shared_t, #smem, mutable, 1x64x128>, !ttg.memdesc<128x64xf16, #shared, #smem, mutable, 1x128x64>) -> ()
    nvws.semaphore.release %sem[%c0_i32], %tok [#nvws.async_op<tc5mma>] {loop.cluster = 6 : i32, loop.stage = 2 : i32, ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x64xf16, #shared, #smem, mutable>], 2>, !ttg.async.token
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @dual_tma_load_order(%desc_a: !tt.tensordesc<tensor<128x64xf16, #shared_desc>>, %desc_b: !tt.tensordesc<tensor<128x64xf16, #shared_desc>>) {
    ...
    %sem = nvws.semaphore.create %buf_a, %buf_b false : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared_desc, #smem, mutable>, !ttg.memdesc<2x128x64xf16, #shared_desc, #smem, mutable>], 2>
    %tok = nvws.semaphore.acquire %sem[%c0_i32, %cm1_i32] {loop.cluster = 7 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>} : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared_desc, #smem, mutable>, !ttg.memdesc<2x128x64xf16, #shared_desc, #smem, mutable>], 2> -> !ttg.async.token
    %views:2 = nvws.semaphore.buffer %sem[%c0_i32], %tok {loop.cluster = 7 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>} : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared_desc, #smem, mutable>, !ttg.memdesc<2x128x64xf16, #shared_desc, #smem, mutable>], 2>, !ttg.async.token -> !ttg.memdesc<128x64xf16, #shared_desc, #smem, mutable, 1x128x64>, !ttg.memdesc<128x64xf16, #shared_desc, #smem, mutable, 1x128x64>
    ...
    // CHECK-COUNT-2: ttg.memdesc_index %{{.*}}[%c0_i32] {loop.cluster = 7 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>} : !ttg.memdesc<2x128x64xf16, #shared, #smem, mutable> -> !ttg.memdesc<128x64xf16, #shared, #smem, mutable>
    // CHECK: [[TMA_MBAR:%.*]] = ttg.memdesc_index %{{.*}}[%c0_i32] {loop.cluster = 7 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>} : !ttg.memdesc<2x1xi64, #shared1, #smem, mutable> -> !ttg.memdesc<1xi64, #shared1, #smem, mutable>
    ...
    nvws.semaphore.release %sem[%c0_i32], %tok [#nvws.async_op<tma_load>] {loop.cluster = 7 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>} : !nvws.semaphore<[!ttg.memdesc<2x128x64xf16, #shared_desc, #smem, mutable>, !ttg.memdesc<2x128x64xf16, #shared_desc, #smem, mutable>], 2>, !ttg.async.token
...
// -----
// Phase2Mode combine test: two separate SMEM semaphore pairs feeding the same
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @combine_two_tma_loads(%desc_a: !tt.tensordesc<tensor<128x64xf16, #shared3>>, %desc_b: !tt.tensordesc<tensor<128x64xf16, #shared3>>, %lb: i32, %ub: i32, %step: i32) {
    ...
    // CHECK: ttng.init_barrier {{.*}}, 1
    // CHECK: ttng.init_barrier {{.*}}, 1
    %empty_a = nvws.semaphore.create %alloc_a true : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>], 1>
    %full_a = nvws.semaphore.create %alloc_a false : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>], 1>
    %empty_b = nvws.semaphore.create %alloc_b true : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>], 1>
    %full_b = nvws.semaphore.create %alloc_b false : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>], 1>
    ...
    %0 = scf.for %iv = %lb to %ub step %step iter_args(%acc = %token) -> (!ttg.async.token) : i32 {
      ...
      %tok_a = nvws.semaphore.acquire %empty_a {ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>], 1> -> !ttg.async.token
      %buf_a = nvws.semaphore.buffer %empty_a, %tok_a {ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>], 1>, !ttg.async.token -> !ttg.memdesc<128x64xf16, #shared3, #smem3, mutable>
      ...
      nvws.semaphore.release %full_a, %tok_a [#nvws.async_op<tma_load>] {ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>], 1>, !ttg.async.token
      %tok_b = nvws.semaphore.acquire %empty_b {ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>], 1> -> !ttg.async.token
      %buf_b = nvws.semaphore.buffer %empty_b, %tok_b {ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>], 1>, !ttg.async.token -> !ttg.memdesc<128x64xf16, #shared3, #smem3, mutable>
      ...
      nvws.semaphore.release %full_b, %tok_b [#nvws.async_op<tma_load>] {ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>], 1>, !ttg.async.token
      ...
      %tok_ca = nvws.semaphore.acquire %full_a {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>], 1> -> !ttg.async.token
      %cbuf_a = nvws.semaphore.buffer %full_a, %tok_ca {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>], 1>, !ttg.async.token -> !ttg.memdesc<128x64xf16, #shared3, #smem3, mutable>
      %tok_cb = nvws.semaphore.acquire %full_b {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>], 1> -> !ttg.async.token
      %cbuf_b = nvws.semaphore.buffer %full_b, %tok_cb {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>], 1>, !ttg.async.token -> !ttg.memdesc<128x64xf16, #shared3, #smem3, mutable>
      ...
      nvws.semaphore.release %empty_a, %tok_ca [#nvws.async_op<tc5mma>] {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>], 1>, !ttg.async.token
      nvws.semaphore.release %empty_b, %tok_cb [#nvws.async_op<tc5mma>] {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<1x128x64xf16, #shared3, #smem3, mutable>], 1>, !ttg.async.token
...
// -----
// Phase2Mode: TMEM semaphore ops before/after loop. Pre-loop tmem_store setup,
// in-loop MMA, post-loop tmem_load teardown. TMEM depth=1 (no multi-buffering).
// Ported from lower_aref.mlir @aref_not_in_loop.
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @semaphore_not_in_loop(%arg0: i32, %arg1: i32, %arg2: i32, %arg3: !tt.tensordesc<tensor<128x64xf16, #shared5>>, %arg4: !tt.tensordesc<tensor<128x64xf16, #shared5>>) {
    ...
    // CHECK: [[MBAR_F:%.*]] = ttg.local_alloc : () -> !ttg.memdesc<1x1xi64,
    // CHECK: ttng.init_barrier {{.*}}, 1
    %empty = nvws.semaphore.create %result true : !nvws.semaphore<[!ttg.memdesc<1x128x128xf32, #tmem5, #ttng.tensor_memory, mutable>], 1>
    %full = nvws.semaphore.create %result false : !nvws.semaphore<[!ttg.memdesc<1x128x128xf32, #tmem5, #ttng.tensor_memory, mutable>], 1>
    // Pre-loop: wait EMPTY, get buffer view, tmem_store
    // CHECK: ttng.wait_barrier
    // CHECK: [[BUF_PRE:%.*]] = ttg.memdesc_index [[TMEM]]
    // CHECK: ttng.tmem_store {{.*}}, [[BUF_PRE]][]
    %tok = nvws.semaphore.acquire %empty : !nvws.semaphore<[!ttg.memdesc<1x128x128xf32, #tmem5, #ttng.tensor_memory, mutable>], 1> -> !ttg.async.token
    %buf = nvws.semaphore.buffer %empty, %tok : !nvws.semaphore<[!ttg.memdesc<1x128x128xf32, #tmem5, #ttng.tensor_memory, mutable>], 1>, !ttg.async.token -> !ttg.memdesc<128x128xf32, #tmem5, #ttng.tensor_memory, mutable, 1x128x128>
    ...
    scf.for %arg5 = %c0_i32 to %arg0 step %c1_i32 : i32 {
      ...
      %10 = nvws.semaphore.buffer %empty, %tok {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<1x128x128xf32, #tmem5, #ttng.tensor_memory, mutable>], 1>, !ttg.async.token -> !ttg.memdesc<128x128xf32, #tmem5, #ttng.tensor_memory, mutable, 1x128x128>
    ...
    nvws.semaphore.release %full, %tok [#nvws.async_op<tc5mma>] : !nvws.semaphore<[!ttg.memdesc<1x128x128xf32, #tmem5, #ttng.tensor_memory, mutable>], 1>, !ttg.async.token
    %tok_get = nvws.semaphore.acquire %full : !nvws.semaphore<[!ttg.memdesc<1x128x128xf32, #tmem5, #ttng.tensor_memory, mutable>], 1> -> !ttg.async.token
    %buf_get = nvws.semaphore.buffer %full, %tok_get : !nvws.semaphore<[!ttg.memdesc<1x128x128xf32, #tmem5, #ttng.tensor_memory, mutable>], 1>, !ttg.async.token -> !ttg.memdesc<128x128xf32, #tmem5, #ttng.tensor_memory, mutable, 1x128x128>
    ...
    nvws.semaphore.release %empty, %tok_get [#nvws.async_op<none>] : !nvws.semaphore<[!ttg.memdesc<1x128x128xf32, #tmem5, #ttng.tensor_memory, mutable>], 1>, !ttg.async.token
...
// -----
// Phase2Mode: TMEM accumulator with depth=2, standalone semaphore.buffer ops,
// conditional scf.if with MMA + tmem_load/tmem_store. Token chains through
// loop as iter_arg. Ported from lower_aref.mlir @lower_aref_buffer.
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @semaphore_buffer(%arg0: !tt.tensordesc<tensor<128x64xf16, #shared7>>, %arg1: !tt.tensordesc<tensor<64x128xf16, #shared7>>) {
    ...
    %empty = nvws.semaphore.create %result true : !nvws.semaphore<[!ttg.memdesc<2x128x128xf32, #tmem7, #ttng.tensor_memory, mutable>], 2>
    %full = nvws.semaphore.create %result false : !nvws.semaphore<[!ttg.memdesc<2x128x128xf32, #tmem7, #ttng.tensor_memory, mutable>], 2>
    %tok = nvws.semaphore.acquire %empty : !nvws.semaphore<[!ttg.memdesc<2x128x128xf32, #tmem7, #ttng.tensor_memory, mutable>], 2> -> !ttg.async.token
    %buf0 = nvws.semaphore.buffer %empty, %tok : !nvws.semaphore<[!ttg.memdesc<2x128x128xf32, #tmem7, #ttng.tensor_memory, mutable>], 2>, !ttg.async.token -> !ttg.memdesc<128x128xf32, #tmem7, #ttng.tensor_memory, mutable, 2x128x128>
    ...
    %3 = scf.for %arg2 = %c0_i32 to %c32_i32 step %c1_i32 iter_args(%arg3 = %tok) -> (!ttg.async.token) : i32 {
      ...
      %9 = nvws.semaphore.buffer %empty, %arg3 {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<2x128x128xf32, #tmem7, #ttng.tensor_memory, mutable>], 2>, !ttg.async.token -> !ttg.memdesc<128x128xf32, #tmem7, #ttng.tensor_memory, mutable, 2x128x128>
      ...
      %12 = scf.if %11 -> (!ttg.async.token) {
        ...
        nvws.semaphore.release %full, %arg3 [#nvws.async_op<tc5mma>] {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<2x128x128xf32, #tmem7, #ttng.tensor_memory, mutable>], 2>, !ttg.async.token
        %tok_get = nvws.semaphore.acquire %full {ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<2x128x128xf32, #tmem7, #ttng.tensor_memory, mutable>], 2> -> !ttg.async.token
        %buf_get = nvws.semaphore.buffer %full, %tok_get {ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<2x128x128xf32, #tmem7, #ttng.tensor_memory, mutable>], 2>, !ttg.async.token -> !ttg.memdesc<128x128xf32, #tmem7, #ttng.tensor_memory, mutable, 2x128x128>
        ...
        nvws.semaphore.release %empty, %tok_get [#nvws.async_op<none>] {ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<2x128x128xf32, #tmem7, #ttng.tensor_memory, mutable>], 2>, !ttg.async.token
        "acc_user"(%result_3) {ttg.partition = array<i32: 0>} : (tensor<128x128xf32, #blocked7>) -> ()
        // CHECK: ttng.wait_barrier
        // CHECK: scf.yield
        %tok_reacq = nvws.semaphore.acquire %empty {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<2x128x128xf32, #tmem7, #ttng.tensor_memory, mutable>], 2> -> !ttg.async.token
      ...
      %13 = nvws.semaphore.buffer %empty, %12 {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<2x128x128xf32, #tmem7, #ttng.tensor_memory, mutable>], 2>, !ttg.async.token -> !ttg.memdesc<128x128xf32, #tmem7, #ttng.tensor_memory, mutable, 2x128x128>
    ...
    nvws.semaphore.release %full, %3 [#nvws.async_op<none>] : !nvws.semaphore<[!ttg.memdesc<2x128x128xf32, #tmem7, #ttng.tensor_memory, mutable>], 2>, !ttg.async.token
    tt.return
  }
}
