...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  // The region-closing ENTRY release is emitted inside each scf.if branch:
  // the then-branch closes from the C read, the else-branch from the B read.
  ...
  tt.func @depth5_regular_atomic_if(%lb: i32, %ub: i32, %step: i32,
    ...
    // SEMA: [[ENTRY:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] true {pending_count = 1 : i32}
    // SEMA: [[A_FULL:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] false {pending_count = 1 : i32}
    // SEMA: [[A_TO_B:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] false {pending_count = 1 : i32}
    // SEMA: [[B_FULL:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] false {pending_count = 1 : i32}
    // SEMA: [[C_FULL:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] false {pending_count = 1 : i32}
    // ASP: [[ENTRY:%.*]] = nvws.semaphore.create {{.*}} true {pending_count = 1 : i32}
    // ASP: [[A_FULL:%.*]] = nvws.semaphore.create {{.*}} false {pending_count = 1 : i32}
    // ASP: [[A_TO_B:%.*]] = nvws.semaphore.create {{.*}} false {pending_count = 1 : i32}
    // ASP: [[B_FULL:%.*]] = nvws.semaphore.create {{.*}} false {pending_count = 1 : i32}
    // ASP: [[C_FULL:%.*]] = nvws.semaphore.create {{.*}} false {pending_count = 1 : i32}
    ...
    // SEMA: scf.for
    // ASP: %{{[0-9]+}}:6 = scf.for {{.*}} iter_args([[CURSOR:%.*]] = %{{[-A-Za-z0-9_.$#]+}},
    scf.for %iv = %lb to %ub step %step : i32 {
      ...
      // After ASP the scf.if yields B's slot and the C_FULL phase word.
      // SEMA: scf.if
      // ASP: [[IF_RESULTS:%.*]]:2 = scf.if
      scf.if %cond {
        ...
        // The C read closes the region from inside the branch.
        ...
        // SEMA: nvws.semaphore.release [[ENTRY]], [[C_READ_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>}
        ...
        // ASP: nvws.semaphore.release [[ENTRY]][[[B_SLOT]]], [[C_READ_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>}
        // ASP: scf.yield {{.*}} [[B_SLOT]],
        ...
        // Without the C wave, the B read closes the region.
        // SEMA: nvws.semaphore.release [[ENTRY]], [[B_READ_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 0>}
        // ASP: nvws.semaphore.release [[ENTRY]][[[B_SLOT]]], [[B_READ_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 0>}
        // ASP: scf.yield {{.*}} [[B_SLOT]],
      } {loop.cluster = 6 : i32, loop.stage = 2 : i32, ttg.partition = array<i32: 0, 1>, ttg.partition.outputs = []}
      // ASP: scf.yield {{.*}} [[IF_RESULTS]]#0,
  ...
  // full three-iteration orbit; it is emitted inside each scf.if branch (the
  // then-branch closes from the C read, the else-branch from the A/B read).
  ...
  tt.func @depth3_same_owner_atomic_if(%lb: i32, %ub: i32, %step: i32,
    ...
    // SEMA: [[ENTRY:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] true {pending_count = 1 : i32}
    // SEMA: [[AB_FULL:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] false {pending_count = 1 : i32}
    // SEMA: [[C_FULL:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] false {pending_count = 1 : i32}
    // ASP: [[ENTRY:%.*]] = nvws.semaphore.create {{.*}} true {pending_count = 1 : i32}
    // ASP: [[AB_FULL:%.*]] = nvws.semaphore.create {{.*}} false {pending_count = 1 : i32}
    // ASP: [[C_FULL:%.*]] = nvws.semaphore.create {{.*}} false {pending_count = 1 : i32}
    ...
    // SEMA: scf.for
    // ASP: %{{[0-9]+}}:4 = scf.for {{.*}} iter_args([[CURSOR:%.*]] = %{{[-A-Za-z0-9_.$#]+}},
    scf.for %iv = %lb to %ub step %step : i32 {
      ...
      // After ASP the scf.if yields the shared slot and the C_FULL phase word.
      // SEMA: scf.if
      // ASP: [[IF_RESULTS:%.*]]:2 = scf.if
      scf.if %cond {
        ...
        // ASP: ttg.local_load [[C_READ_VIEWS]]#2 {ttg.partition = array<i32: 1>}
        // ASP: nvws.semaphore.release [[ENTRY]][[[AB_SLOT]]], [[C_READ_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>}
        // ASP: scf.yield {{.*}} [[AB_SLOT]],
        %c_read = ttg.local_load %c {ttg.partition = array<i32: 1>} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable> -> tensor<128x128xf16, #blocked128>
        "consume_c"(%c_read) {ttg.partition = array<i32: 1>} : (tensor<128x128xf16, #blocked128>) -> ()
      } else {
        // Without the C wave, the A/B read closes the region.
        // SEMA: nvws.semaphore.release [[ENTRY]], [[AB_READ_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 0>}
        // ASP: nvws.semaphore.release [[ENTRY]][[[AB_SLOT]]], [[AB_READ_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 0>}
        // ASP: scf.yield {{.*}} [[AB_SLOT]],
      } {loop.cluster = 6 : i32, loop.stage = 2 : i32, ttg.partition = array<i32: 0, 1>, ttg.partition.outputs = []}
      // ASP: scf.yield {{.*}} [[IF_RESULTS]]#0,
