...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @circular_start_zero_backing_dominates_creates(
    ...
    // CHECK-NEXT: [[EMPTY:%.*]] = nvws.semaphore.create [[BASE]] true {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>
    // CHECK-NEXT: [[FULL:%.*]] = nvws.semaphore.create [[BASE]] false {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>
