...
module attributes {"ttg.num-warps" = 4 : i32} {
  ...
  tt.func @shared_stage_two_semaphores() {
    ...
    // CHECK: [[TOK0:%.*]] = nvws.semaphore.acquire [[SEM0]][[[S0]], [[CM1]]]
    // CHECK: [[C1_0:%.*]] = arith.constant 1 : i32
    // CHECK: [[SH0:%.*]] = arith.shli [[C1_0]], [[S0]] : i32
    // CHECK: [[PE_NEW:%.*]] = arith.xori [[CM1]], [[SH0]] : i32
    // CHECK: [[TOK1:%.*]] = nvws.semaphore.acquire [[SEM1]][[[S0]], [[PF0]]]
    // CHECK: [[C1_1:%.*]] = arith.constant 1 : i32
    // CHECK: [[SH1:%.*]] = arith.shli [[C1_1]], [[S0]] : i32
    ...
    // CHECK: nvws.semaphore.release [[SEM0]][[[S0]]], [[TOK0]] [#nvws.async_op<none>]
    // CHECK: nvws.semaphore.release [[SEM1]][[[S0]]], [[TOK1]] [#nvws.async_op<none>]
...
module attributes {"ttg.num-warps" = 4 : i32} {
  ...
  tt.func @if_observation(%cond: i1) {
    ...
    // CHECK: [[FALSE:%.*]] = arith.constant false
    %sem = nvws.semaphore.create %buf true : !nvws.semaphore<[!ttg.memdesc<2x1xi32, #shared, #smem, mutable>], 2>
    // CHECK: [[TOK:%.*]] = nvws.semaphore.acquire [[SEM]][[[S0]], [[CM1]]]
    // CHECK: [[C1:%.*]] = arith.constant 1 : i32
    // CHECK: [[SHIFT:%.*]] = arith.shli [[C1]], [[S0]] : i32
    // CHECK: [[PHASE_NEW:%.*]] = arith.xori [[CM1]], [[SHIFT]] : i32
    ...
    // CHECK: scf.yield [[S0]], {{%.*}} : i32, i1
    // CHECK: } else {
    // CHECK: scf.yield [[S0]], [[FALSE]] : i32, i1
    scf.if %cond {
    ...
    // CHECK: [[NEXT:%.*]] = arith.addi [[IF]]#0, {{%.*}} : i32
    // CHECK: [[WRAP:%.*]] = arith.cmpi eq, [[NEXT]], {{%.*}} : i32
    // CHECK: nvws.semaphore.release [[SEM]][[[S0]]], [[TOK]] [#nvws.async_op<none>]
...
module attributes {"ttg.num-warps" = 4 : i32} {
  ...
  tt.func @two_consumers(%arg0: i32, %arg1: i32, %arg2: i32) {
    ...
    // CHECK: [[LOOP:%.*]]:4 = scf.for {{.*}} iter_args([[STAGE:%.*]] = [[C0]], [[OBS:%.*]] = [[FALSE]], [[PE:%.*]] = [[CM1]], [[PF:%.*]] = [[PF0]]) -> (i32, i1, i32, i32)
    scf.for %arg3 = %arg0 to %arg1 step %arg2  : i32 {
      %2 = "op_a"() {ttg.partition = array<i32: 0>} : () -> tensor<1xi32, #blocked>
      // CHECK: [[PTOK:%.*]] = nvws.semaphore.acquire [[EMPTY]][[[STAGE]], [[PE]]] {ttg.partition = array<i32: 0>}
      ...
      // CHECK: [[GTOK1:%.*]] = nvws.semaphore.acquire [[FULL]][[[ADV_STAGE]], [[PF]]] {ttg.partition = array<i32: 1>}
      // CHECK: [[C1G1:%.*]] = arith.constant {ttg.partition = array<i32: 1>} 1 : i32
      // CHECK: [[GSHIFT1:%.*]] = arith.shli [[C1G1]], [[ADV_STAGE]] {ttg.partition = array<i32: 1>} : i32
      // CHECK: [[PF_MID:%.*]] = arith.xori [[PF]], [[GSHIFT1]] {ttg.partition = array<i32: 1>} : i32
      // CHECK: nvws.semaphore.release [[EMPTY]][[[ADV_STAGE]]], [[GTOK1]] [#nvws.async_op<none>] {ttg.partition = array<i32: 1>}
      ...
      // CHECK: [[GTOK2:%.*]] = nvws.semaphore.acquire [[FULL]][[[ADV_STAGE]], [[PF_MID]]] {ttg.partition = array<i32: 2>}
      // CHECK: [[C1G2:%.*]] = arith.constant {ttg.partition = array<i32: 2>} 1 : i32
      // CHECK: [[GSHIFT2:%.*]] = arith.shli [[C1G2]], [[ADV_STAGE]] {ttg.partition = array<i32: 2>} : i32
      // CHECK: [[PF_NEW:%.*]] = arith.xori [[PF_MID]], [[GSHIFT2]] {ttg.partition = array<i32: 2>} : i32
      // CHECK: nvws.semaphore.release [[EMPTY]][[[ADV_STAGE]]], [[GTOK2]] [#nvws.async_op<none>] {ttg.partition = array<i32: 2>}
      ...
      // CHECK: scf.yield {ttg.partition = array<i32: 0, 1, 2>} [[ADV_STAGE]], {{%.*}}, [[PE_NEW]], [[PF_NEW]] : i32, i1, i32, i32
    // CHECK-NEXT: } {ttg.partition = array<i32: 0, 1, 2>, ttg.partition.outputs = [array<i32: 0, 1, 2>, array<i32: 0, 1, 2>, array<i32: 0, 1, 2>, array<i32: 0, 1, 2>], ttg.partition.stages = [0 : i32, 2 : i32, 2 : i32], ttg.warp_specialize.tag = 0 : i32}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @warp_specialize_tma_matmul(%arg0: i32, %arg1: i32, %arg2: i32, %arg3: !tt.tensordesc<tensor<128x64xf16, #shared>>, %arg4: !tt.tensordesc<tensor<128x64xf16, #shared>>) {
    ...
    // CHECK: [[TOK0:%.*]] = nvws.semaphore.acquire [[EMPTY]][[[S0_INIT]], [[CM1]]]
    // CHECK: [[C1_PRE:%.*]] = arith.constant 1 : i32
    // CHECK: [[SH_PRE:%.*]] = arith.shli [[C1_PRE]], [[S0_INIT]] : i32
    ...
    // CHECK: [[ADV_OBS:%.*]] = arith.select {{%.*}}, {{%.*}}, [[FALSE]]
    ...
    // CHECK: [[LOOP:%.*]]:2 = scf.for {{.*}} iter_args([[LSTAGE:%.*]] = [[ADV_STAGE]], [[LOBS:%.*]] = [[ADV_OBS]]) -> (i32, i1)
    scf.for %arg5 = %c0_i32 to %arg0 step %c1_i32  : i32 {
      ...
      // CHECK: nvws.semaphore.buffer [[EMPTY]][[[S0_INIT]]], [[TOK0]] {ttg.partition = array<i32: 1>}
      ...
      // CHECK: [[L_ADV:%.*]] = arith.select {{%.*}}, {{%.*}}, [[LSTAGE]]
      // CHECK: [[L_OBS:%.*]] = arith.select {{%.*}}, {{%.*}}, [[LOBS]]
      // CHECK: scf.yield {ttg.partition = array<i32: 0, 1, 2>} [[L_ADV]], [[L_OBS]] : i32, i1
    ...
    // CHECK: nvws.semaphore.release [[FULL]][[[S0_INIT]]], [[TOK0]] [#nvws.async_op<tc5mma>]
    ...
    // CHECK: [[TOK1:%.*]] = nvws.semaphore.acquire [[FULL]][[[LOOP]]#0, [[PF_INIT]]]
    // CHECK: [[C1_POST:%.*]] = arith.constant 1 : i32
    // CHECK: [[SH_POST:%.*]] = arith.shli [[C1_POST]], [[LOOP]]#0 : i32
    ...
    // CHECK: nvws.semaphore.release [[EMPTY]][[[LOOP]]#0], [[TOK1]] [#nvws.async_op<none>]
  ...
  tt.func @matmul_tma_acc_with_unconditional_user(%arg0: !tt.tensordesc<tensor<128x64xf16, #shared>>, %arg1: !tt.tensordesc<tensor<64x128xf16, #shared>>) {
    ...
    // CHECK: [[PF_INIT:%.*]] = arith.constant 0 : i32
    // Pre-loop acquire and phase update
    // CHECK: [[PRETOK:%.*]] = nvws.semaphore.acquire [[EMPTY]][[[S0_INIT]], [[CM1]]]
    // CHECK: [[C1_PRE:%.*]] = arith.constant 1 : i32
    // CHECK: [[SH_PRE:%.*]] = arith.shli [[C1_PRE]], [[S0_INIT]] : i32
    // CHECK: [[PE_PRE:%.*]] = arith.xori [[CM1]], [[SH_PRE]] : i32
    // Pre-loop stage advancement
    // CHECK: [[ADV_S:%.*]] = arith.select {{%.*}}, {{%.*}}, [[S0_INIT]]
    // CHECK: [[ADV_O:%.*]] = arith.select {{%.*}}, {{%.*}}, [[FALSE]]
    ...
    // CHECK: [[FOR:%.*]]:5 = scf.for {{.*}} iter_args([[FTOK:%.*]] = [[PRETOK]], [[FS:%.*]] = [[ADV_S]], [[FO:%.*]] = [[ADV_O]], [[FPF:%.*]] = [[PF_INIT]], [[FPE:%.*]] = [[PE_PRE]]) -> (!ttg.async.token, i32, i1, i32, i32)
    %3 = scf.for %arg2 = %c0_i32 to %c32_i32 step %c1_i32 iter_args(%arg3 = %token) -> (!ttg.async.token)  : i32 {
      ...
      // CHECK: [[GTOK:%.*]] = nvws.semaphore.acquire [[FULL]][[[LADV]], [[FPF]]] {ttg.partition = array<i32: 0>}
      // CHECK: [[C1_G:%.*]] = arith.constant {ttg.partition = array<i32: 0>} 1 : i32
      // CHECK: [[GSH:%.*]] = arith.shli [[C1_G]], [[LADV]] {ttg.partition = array<i32: 0>} : i32
      ...
      // CHECK: nvws.semaphore.release [[EMPTY]][[[LADV]]], [[GTOK]] [#nvws.async_op<none>] {ttg.partition = array<i32: 0>}
      ...
      // CHECK: [[PTOK:%.*]] = nvws.semaphore.acquire [[EMPTY]][[[LADV]], [[FPE]]] {ttg.partition = array<i32: 1>}
      // CHECK: [[C1_P:%.*]] = arith.constant {ttg.partition = array<i32: 1>} 1 : i32
      // CHECK: [[PSH:%.*]] = arith.shli [[C1_P]], [[LADV]] {ttg.partition = array<i32: 1>} : i32
      ...
      // CHECK: [[LOBS2:%.*]] = arith.select {{%.*}}, {{%.*}}, {{%.*}}
      // CHECK: scf.yield {ttg.partition = array<i32: 0, 1, 2>} [[PTOK]], [[LADV2]], [[LOBS2]], [[FPF_NEW]], [[FPE_NEW]]
  ...
  tt.func @assign_stage_buffer(%arg0: !tt.tensordesc<tensor<128x64xf16, #shared>>, %arg1: !tt.tensordesc<tensor<64x128xf16, #shared>>) {
    ...
    %3 = scf.for %arg2 = %c0_i32 to %c32_i32 step %c1_i32 iter_args(%arg3 = %token) -> (!ttg.async.token)  : i32 {
      ...
      %12 = scf.if %11 -> (!ttg.async.token) {
        ...
        // CHECK: scf.yield {ttg.partition = array<i32: 0, 1>} [[PTOK]], [[LADV]], {{%.*}}, [[FPF_T]], [[FPE_T]]
        scf.yield %token_6 : !ttg.async.token
      } else {
        // CHECK: scf.yield {ttg.partition = array<i32: 0, 1>} [[FTOK]], [[LADV]], [[LOBS]], [[FPF]], [[FPE]]
      ...
      // CHECK: scf.yield {ttg.partition = array<i32: 0, 1, 2>} [[IF]]#0, [[LADV2]], {{%.*}}, [[IF]]#3, [[IF]]#4
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func public @attention_forward(%arg0: !ttg.memdesc<256x64xf16, #shared, #smem>, %arg1: !tt.tensordesc<tensor<64x64xf16, #shared>>, %arg2: !tt.tensordesc<tensor<64x64xf16, #shared>>, %arg3: f32, %arg4: i32) {
    ...
    // CHECK: [[LOOP:%.*]]:16 = scf.for {{.*}} iter_args({{.*}}) -> (tensor<256xf32, #ttg.slice<{dim = 1, parent = #blocked}>>, tensor<256xf32, #ttg.slice<{dim = 1, parent = #blocked}>>, !ttg.async.token, !ttg.async.token, i32, i1, i32, i1, i32, i1, i32, i32, i32, i32, i32, i32)
    %5:4 = scf.for %arg5 = %c0_i32 to %arg4 step %c64_i32 iter_args(%arg6 = %cst, %arg7 = %cst_1, %arg8 = %token, %arg9 = %token_4) -> (tensor<256xf32, #ttg.slice<{dim = 1, parent = #blocked}>>, tensor<256xf32, #ttg.slice<{dim = 1, parent = #blocked}>>, !ttg.async.token, !ttg.async.token)  : i32 {
      ...
      // CHECK: [[GTOK0:%.*]] = nvws.semaphore.acquire [[F0]][[[S0_ADV]], {{%.*}}] {ttg.partition = array<i32: 0>}
      // CHECK: [[C1_G0:%.*]] = arith.constant {ttg.partition = array<i32: 0>} 1 : i32
      // CHECK: [[GSH0:%.*]] = arith.shli [[C1_G0]], [[S0_ADV]] {ttg.partition = array<i32: 0>} : i32
      ...
      // CHECK: ttng.tmem_load {{.*}} {ttg.partition = array<i32: 0>}
      // CHECK: nvws.semaphore.release [[E0]][[[S0_ADV]]], [[GTOK0]] [#nvws.async_op<none>] {ttg.partition = array<i32: 0>}
      ...
      // CHECK: nvws.semaphore.acquire [[E0]][[[S0_ADV]], {{%.*}}] {ttg.partition = array<i32: 1>}
    ...
    // CHECK: nvws.semaphore.release [[F1]][[[LOOP]]#6], [[LOOP]]#3 [#nvws.async_op<tc5mma>]
    ...
    // CHECK: [[TOK_END:%.*]] = nvws.semaphore.acquire [[F1]][[[LOOP]]#6, [[LOOP]]#12]
    // CHECK: [[C1_END:%.*]] = arith.constant 1 : i32
    // CHECK: [[SH_END:%.*]] = arith.shli [[C1_END]], [[LOOP]]#6 : i32
    // CHECK: [[PE_END:%.*]] = arith.xori [[LOOP]]#12, [[SH_END]] : i32
    // CHECK: ttng.tmem_load
    // CHECK: nvws.semaphore.release [[E1]][[[LOOP]]#6], [[TOK_END]] [#nvws.async_op<none>]
  ...
  // CHECK-LABEL: @matmul_tma_acc_with_conditional_user
    ...
    %3 = scf.for %arg2 = %c0_i32 to %c32_i32 step %c1_i32 iter_args(%arg3 = %token) -> (!ttg.async.token)  : i32 {
      ...
      %12 = scf.if %11 -> (!ttg.async.token) {
        ...
        nvws.aref.put.exit %0, %arg3 [#nvws.async_op<tc5mma>] {ttg.partition = array<i32: 1>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>, !ttg.async.token
        // CHECK: [[GTOK:%.*]] = nvws.semaphore.acquire [[FULL]][[[GS:%[-a-zA-Z0-9_#]+]], [[GP_IN:%[-a-zA-Z0-9_#]+]]] {ttg.partition = array<i32: 0>}
        // CHECK: [[GSH:%.*]] = arith.shli {{%.*}}, [[GS]] {ttg.partition = array<i32: 0>} : i32
        ...
        nvws.aref.get.exit %0, %token_2 [#nvws.async_op<none>] {ttg.partition = array<i32: 0>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>, !ttg.async.token
        "acc_user"(%result_3) {ttg.partition = array<i32: 0>} : (tensor<128x128xf32, #blocked>) -> ()
        // CHECK: [[PTOK:%.*]] = nvws.semaphore.acquire [[EMPTY]][[[PS:%[-a-zA-Z0-9_#]+]], [[PP_IN:%[-a-zA-Z0-9_#]+]]] {ttg.partition = array<i32: 1>}
        // CHECK: [[PSH:%.*]] = arith.shli {{%.*}}, [[PS]] {ttg.partition = array<i32: 1>} : i32
        // CHECK: [[PP_OUT:%.*]] = arith.xori [[PP_IN]], [[PSH]] {ttg.partition = array<i32: 1>} : i32
        %buffers_5, %token_6 = nvws.aref.put.enter %0 {ttg.partition = array<i32: 1>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]> -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 2x128x128>, !ttg.async.token
        // CHECK: scf.yield {ttg.partition = array<i32: 0, 1>} [[PTOK]], [[PS]], {{%.*}}, [[GP_OUT]], [[PP_OUT]]
        scf.yield %token_6 : !ttg.async.token
      } else {
        scf.yield %arg3 : !ttg.async.token
      } {ttg.partition = array<i32: 0, 1>, ttg.partition.outputs = [array<i32: 1>]}
      ...
      %13 = nvws.aref.buffer %0, %12 {ttg.partition = array<i32: 1>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 2x128x128>
      %14 = ttng.tmem_store %cst, %13[], %true {ttg.partition = array<i32: 1>} : tensor<128x128xf32, #blocked> -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 2x128x128>
      // CHECK: scf.yield {ttg.partition = array<i32: 0, 1, 2>} [[IF]]#0, {{%.*}}, {{%.*}}, [[IF]]#3, [[IF]]#4
      scf.yield %12 : !ttg.async.token
    } {tt.num_stages = 2 : i32, tt.warp_specialize, ttg.partition.stages = [0 : i32, 1 : i32, 0 : i32], ttg.warp_specialize.tag = 5 : i32, ttg.partition = array<i32: 0, 1, 2>, ttg.partition.outputs = [array<i32: 1>]}
    // CHECK: } {tt.num_stages = 2 : i32, tt.warp_specialize, ttg.partition = array<i32: 0, 1, 2>, ttg.partition.outputs = [array<i32: 1>, array<i32: 0, 1, 2>, array<i32: 0, 1, 2>, array<i32: 0, 1, 2>, array<i32: 0, 1, 2>], ttg.partition.stages = [0 : i32, 1 : i32, 0 : i32], ttg.warp_specialize.tag = 5 : i32}
    // CHECK: nvws.semaphore.release [[FULL]][[[FOR]]#1], [[FOR]]#0 [#nvws.async_op<none>]
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func public @matmul_tma_persistent_ws_kernel(%arg0: !tt.ptr<f8E4M3FN> {tt.divisibility = 16 : i32}, %arg1: !tt.ptr<f8E4M3FN> {tt.divisibility = 16 : i32}, %arg2: !tt.ptr<f8E4M3FN> {tt.divisibility = 16 : i32}, %arg3: i32 {tt.divisibility = 16 : i32}, %arg4: i32 {tt.divisibility = 16 : i32}, %arg5: i32 {tt.divisibility = 16 : i32}, %arg6: i32 {tt.divisibility = 16 : i32}, %arg7: i32 {tt.divisibility = 16 : i32}, %arg8: i32 {tt.divisibility = 16 : i32}) attributes {noinline = false} {
    ...
    // CHECK: [[OUTER:%.*]]:8 = scf.for {{.*}} iter_args({{.*}}) -> (i32, i1, i32, i1, i32, i32, i32, i32)
    scf.for %arg9 = %6 to %13 step %c148_i32  : i32 {
      ...
      // CHECK: [[ATOK0:%.*]] = nvws.semaphore.acquire [[ACC_EMPTY]][[[ASTAGE0:%[-a-zA-Z0-9_#]+]], [[APHASE0_IN:%[-a-zA-Z0-9_#]+]]] {ttg.partition = array<i32: 0>}
      ...
      // CHECK: [[ATOK1:%.*]] = nvws.semaphore.acquire [[ACC_FULL]][[[ASTAGE1:%[-a-zA-Z0-9_#]+]], [[APHASE1_IN:%[-a-zA-Z0-9_#]+]]] {ttg.partition = array<i32: 1>}
      ...
      // CHECK: [[INNER:%.*]]:7 = scf.for {{.*}} iter_args({{.*}}) -> (i1, i32, i1, i32, i1, i32, i32)
      %32 = scf.for %arg10 = %c0_i32 to %12 step %c1_i32 iter_args(%arg11 = %false) -> (i1)  : i32 {
        ...
        // CHECK: [[ABTOK_P:%.*]] = nvws.semaphore.acquire [[AB_EMPTY]][[[ABSTAGE_P:%[-a-zA-Z0-9_#]+]], [[ABPHASE_P_IN:%[-a-zA-Z0-9_#]+]]] {loop.cluster = 2 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>}
        ...
        // CHECK: [[ABTOK_C:%.*]] = nvws.semaphore.acquire [[AB_FULL]][[[ABSTAGE_C:%[-a-zA-Z0-9_#]+]], [[ABPHASE_C_IN:%[-a-zA-Z0-9_#]+]]] {ttg.partition = array<i32: 1>}
      ...
      // CHECK: [[ATOK2:%.*]] = nvws.semaphore.acquire [[ACC_EMPTY]][[[ASTAGE2:%[-a-zA-Z0-9_#]+]], [[APHASE2_IN:%[-a-zA-Z0-9_#]+]]] {ttg.partition = array<i32: 0>}
      ...
      // CHECK: [[ATOK3:%.*]] = nvws.semaphore.acquire [[ACC_FULL]][[[ASTAGE3:%[-a-zA-Z0-9_#]+]], [[APHASE3_IN:%[-a-zA-Z0-9_#]+]]] {ttg.partition = array<i32: 1>}
      ...
      // CHECK: scf.yield {ttg.partition = array<i32: 0, 1, 2>} {{%.*}}, {{%.*}}, [[ASTAGE3]], {{%.*}}, [[APHASE2_OUT]], [[APHASE3_OUT]], {{%.*}}, {{%.*}}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @for_loop_control_operand_ppg(%lb: i32, %ub: i32, %step: i32, %ptr0: !tt.ptr<i32>) {
    ...
    // CHECK: [[EMPTY:%.*]] = nvws.semaphore.create {{.*}} true : <[!ttg.memdesc<1x128x128xf32
    // CHECK: [[FULL:%.*]] = nvws.semaphore.create {{.*}} false : <[!ttg.memdesc<1x128x128xf32
    // CHECK: [[TOK0:%.*]] = nvws.semaphore.acquire [[EMPTY]][[[S0:%[-a-zA-Z0-9_#]+]], [[P0_IN:%[-a-zA-Z0-9_#]+]]]
    ...
    // CHECK: [[FOR0:%.*]]:5 = scf.for {{.*}} iter_args({{.*}}) -> (!ttg.async.token, i32, i1, i32, i32)
    %tok0 = scf.for %iv0 = %lb to %ub step %step iter_args(%tok1 = %tok) -> (!ttg.async.token) : i32 {
      ...
      // CHECK: [[FOR1:%.*]]:3 = scf.for {{.*}} iter_args({{.*}}) -> (!ttg.async.token, i32, i1)
      ...
      // CHECK: [[TOK1:%.*]] = nvws.semaphore.acquire [[FULL]][[[S1:%[-a-zA-Z0-9_#]+]], [[P1_IN:%[-a-zA-Z0-9_#]+]]] {ttg.partition = array<i32: 1>}
      ...
      // CHECK: [[TOK2:%.*]] = nvws.semaphore.acquire [[EMPTY]][[[S1]], [[P2_IN:%[-a-zA-Z0-9_#]+]]] {ttg.partition = array<i32: 2>}
      // CHECK: [[SH2:%.*]] = arith.shli {{%.*}}, [[S1]] {ttg.partition = array<i32: 2>} : i32
      ...
      // CHECK: scf.yield {ttg.partition = array<i32: 0, 1, 2>} [[TOK2]], [[S1]], {{%.*}}, [[P1_OUT]], [[P2_OUT]]
    ...
    // CHECK: [[TOK_END:%.*]] = nvws.semaphore.acquire [[FULL]][[[S_END:%[-a-zA-Z0-9_#]+]], [[P_END_IN:%[-a-zA-Z0-9_#]+]]]
