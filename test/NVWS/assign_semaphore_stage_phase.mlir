...
module attributes {"ttg.num-warps" = 4 : i32} {
  ...
  tt.func @assign_stage_basic(%lb: i32, %ub: i32, %step: i32) {
    ...
    scf.for %i = %lb to %ub step %step : i32 {
      ...
      // CHECK: [[TOK:%.*]] = nvws.semaphore.acquire [[SEM]][[[STAGE]], [[PHASE_NEW]]] {ttg.partition = array<i32: 0>}
...
module attributes {"ttg.num-warps" = 4 : i32} {
  ...
  tt.func @if_observation(%cond: i1, %lb: i32, %ub: i32, %step: i32) {
    ...
    scf.for %i = %lb to %ub step %step : i32 {
      ...
      // CHECK: [[STEP:%.*]] = arith.constant {ttg.partition = array<i32: 0>} 1 : i32
      // CHECK: [[ZERO:%.*]] = arith.constant {ttg.partition = array<i32: 0>} 0 : i32
      // CHECK: [[DEPTH:%.*]] = arith.constant {ttg.partition = array<i32: 0>} 2 : i32
      // CHECK: [[NEXT:%.*]] = arith.addi [[STAGE]], [[STEP]] {ttg.partition = array<i32: 0>} : i32
      // CHECK: [[WRAP:%.*]] = arith.cmpi eq, [[NEXT]], [[DEPTH]] {ttg.partition = array<i32: 0>} : i32
      // CHECK: [[NEW_STAGE:%.*]] = arith.select [[WRAP]], [[ZERO]], [[NEXT]] {ttg.partition = array<i32: 0>} : i32
      // Phase flip BEFORE acquire
      // CHECK: [[C1:%.*]] = arith.constant {ttg.partition = array<i32: 0>} 1 : i32
      ...
      %tok = nvws.semaphore.acquire %sem {ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<2x1xi32, #shared, #smem, mutable>], 2> -> !ttg.async.token
      // CHECK: [[BUF:%.*]] = nvws.semaphore.buffer [[SEM]][[[NEW_STAGE]]], [[TOK]] {ttg.partition = array<i32: 0>}
      // CHECK: ttg.local_load [[BUF]] {ttg.partition = array<i32: 0>}
      // CHECK: ttg.local_store {{%.*}}, [[BUF]] {ttg.partition = array<i32: 0>}
      %view = nvws.semaphore.buffer %sem, %tok {ttg.partition = array<i32: 0>} : !nvws.semaphore<[!ttg.memdesc<2x1xi32, #shared, #smem, mutable>], 2>, !ttg.async.token -> !ttg.memdesc<1xi32, #shared, #smem, mutable, 2x1>
...
module attributes {"ttg.target" = "cuda:0", "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 1 : i32, "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func @semaphore_lowering(%d : !ttg.memdesc<3x64x16xf16, #shared0, #smem>,
    ...
    scf.for %i = %lb to %ub step %c1_i32 : i32{
      ...
      // CHECK: [[PC1_P0:%.*]] = arith.constant {ttg.partition = array<i32: 0>} 1 : i32
      // CHECK: [[PSH0:%.*]] = arith.shli [[PC1_P0]], [[S0]] {ttg.partition = array<i32: 0>} : i32
      ...
      // CHECK: [[PBUF0:%.*]]:2 = nvws.semaphore.buffer [[E0]][[[S0]]], [[PTOK0]] {ttg.partition = array<i32: 0>}
      // CHECK: "op1"([[PBUF0]]#0) {ttg.partition = array<i32: 0>}
      // CHECK: "op2"([[PBUF0]]#1) {ttg.partition = array<i32: 0>}
      ...
      // CHECK: [[GSH0:%.*]] = arith.shli [[GC1_C0]], [[S0]] {ttg.partition = array<i32: 1>} : i32
      ...
      // CHECK: [[GBUF0:%.*]]:2 = nvws.semaphore.buffer [[F0]][[[S0]]], [[GTOK0]] {ttg.partition = array<i32: 1>}
      // CHECK: "op3"([[GBUF0]]#0, [[GBUF0]]#1) {ttg.partition = array<i32: 1>}
      ...
      // CHECK: [[EC1_P1:%.*]] = arith.constant {ttg.partition = array<i32: 0>} 1 : i32
      // CHECK: [[ESH_P1:%.*]] = arith.shli [[EC1_P1]], [[S1]] {ttg.partition = array<i32: 0>} : i32
      // CHECK: [[PE1_NEW:%.*]] = arith.xori [[PE1]], [[ESH_P1]] {ttg.partition = array<i32: 0>} : i32
      // CHECK: [[ETOK_P1:%.*]] = nvws.semaphore.acquire [[E1]][[[S1]], [[PE1_NEW]]] {ttg.partition = array<i32: 0>}
      // CHECK: [[EBUF_P1:%.*]]:2 = nvws.semaphore.buffer [[E1]][[[S1]]], [[ETOK_P1]] {ttg.partition = array<i32: 0>}
      // CHECK: "op4"([[EBUF_P1]]#0, [[EBUF_P1]]#1) {ttg.partition = array<i32: 0>}
      // CHECK: nvws.semaphore.release [[F1]][[[S1]]], [[ETOK_P1]] [#nvws.async_op<none>] {ttg.partition = array<i32: 0>}
      // Phase flip pf1 BEFORE acquire F1
      // CHECK: [[FC1_C1:%.*]] = arith.constant {ttg.partition = array<i32: 1>} 1 : i32
      // CHECK: [[FSH_C1:%.*]] = arith.shli [[FC1_C1]], [[S1]] {ttg.partition = array<i32: 1>} : i32
      // CHECK: [[PF1_NEW:%.*]] = arith.xori [[PF1]], [[FSH_C1]] {ttg.partition = array<i32: 1>} : i32
      // CHECK: [[FTOK_C1:%.*]] = nvws.semaphore.acquire [[F1]][[[S1]], [[PF1_NEW]]] {ttg.partition = array<i32: 1>}
      // CHECK: [[FBUF_C1:%.*]]:2 = nvws.semaphore.buffer [[F1]][[[S1]]], [[FTOK_C1]] {ttg.partition = array<i32: 1>}
      // CHECK: "op5"([[FBUF_C1]]#0, [[FBUF_C1]]#1) {ttg.partition = array<i32: 1>}
      // CHECK: nvws.semaphore.release [[E1]][[[S1]]], [[FTOK_C1]] [#nvws.async_op<tc5mma>] {ttg.partition = array<i32: 1>}
      // CHECK: scf.yield {ttg.partition = array<i32: 0, 1>} [[S1]], [[PE1_NEW]], [[PF1_NEW]] : i32, i32, i32
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @warp_specialize_tma_matmul(%arg0: i32, %arg1: i32, %arg2: i32, %arg3: !tt.tensordesc<tensor<128x64xf16, #shared>>, %arg4: !tt.tensordesc<tensor<128x64xf16, #shared>>) {
    ...
    // CHECK: {{.*}} = nvws.semaphore.buffer [[EMPTY]][[[PRE_STAGE]]], [[TOK0]]
    ...
    // CHECK: {{.*}} = nvws.semaphore.buffer [[FULL]][[[PRE_STAGE]]], [[TOK1]]
  ...
  tt.func @matmul_tma_acc_with_unconditional_user(%arg0: !tt.tensordesc<tensor<128x64xf16, #shared>>, %arg1: !tt.tensordesc<tensor<64x128xf16, #shared>>) {
    ...
    // CHECK: {{.*}} = nvws.semaphore.buffer [[EMPTY]][[[PRE_STAGE]]], [[PRETOK]]
    ...
    // CHECK: [[FOR:%.*]]:4 = scf.for {{.*}} iter_args([[FTOK:%.*]] = [[PRETOK]], [[FSTAGE:%.*]] = [[PRE_STAGE]], [[FPE:%.*]] = [[PF1_OUT]], [[FPF:%.*]] = [[PF_INIT]]) -> (!ttg.async.token, i32, i32, i32)
    %3 = scf.for %arg2 = %c0_i32 to %c32_i32 step %c1_i32 iter_args(%arg3 = %token) -> (!ttg.async.token)  : i32 {
      ...
      // CHECK: {{.*}} = nvws.semaphore.buffer [[FULL]][[[FSTAGE]]], [[GTOK]] {ttg.partition = array<i32: 0>}
      ...
      // CHECK: {{.*}} = nvws.semaphore.buffer [[EMPTY]][[[NEXT_STAGE]]], [[PTOK]] {ttg.partition = array<i32: 1>}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @assign_stage_buffer(%arg0: !tt.tensordesc<tensor<128x64xf16, #shared>>, %arg1: !tt.tensordesc<tensor<64x128xf16, #shared>>) {
    ...
    // CHECK: [[FULL:%.*]] = nvws.semaphore.create %{{.*}} false {nvws.use_single_phase = false}
    // Pre-loop: stage advance, phase flips, acquire EMPTY
    // CHECK: [[S_INIT:%.*]] = arith.constant 1 : i32
    // CHECK: [[PE_INIT:%.*]] = arith.constant 0 : i32
    // CHECK: [[PF_INIT:%.*]] = arith.constant -1 : i32
    ...
    // CHECK: [[PRE_STAGE:%.*]] = arith.select [[SA_CMP]], [[SA_ZERO]], [[SA_ADD]] : i32
    // CHECK: [[PF1_C1:%.*]] = arith.constant 1 : i32
    // CHECK: [[PF1_SH:%.*]] = arith.shli [[PF1_C1]], [[PRE_STAGE]] : i32
    // CHECK: [[PF1_OUT:%.*]] = arith.xori [[PE_INIT]], [[PF1_SH]] : i32
    // CHECK: [[PE1_C1:%.*]] = arith.constant 1 : i32
    // CHECK: [[PE1_SH:%.*]] = arith.shli [[PE1_C1]], [[PRE_STAGE]] : i32
    // CHECK: [[PE_PRE:%.*]] = arith.xori [[PE_INIT]], [[PE1_SH]] : i32
    // CHECK: [[PRETOK:%.*]] = nvws.semaphore.acquire [[EMPTY]][[[PRE_STAGE]], [[PE_PRE]]]
    // CHECK: {{.*}} = nvws.semaphore.buffer [[EMPTY]][[[PRE_STAGE]]], [[PRETOK]]
    // CHECK: [[BUF_INIT:%.*]] = nvws.semaphore.buffer [[EMPTY]][[[PRE_STAGE]]], [[PRETOK]]
    // CHECK: ttng.tmem_store {{%.*}}, [[BUF_INIT]][], {{%.*}}
    ...
    %3 = scf.for %arg2 = %c0_i32 to %c32_i32 step %c1_i32 iter_args(%arg3 = %token) -> (!ttg.async.token)  : i32 {
      ...
      %12 = scf.if %11 -> (!ttg.async.token) {
        ...
        // CHECK: [[GC1:%.*]] = arith.constant {ttg.partition = array<i32: 0>} 1 : i32
        // CHECK: [[GSH:%.*]] = arith.shli [[GC1]], [[FSTAGE]] {ttg.partition = array<i32: 0>} : i32
        ...
        // CHECK: [[PC1:%.*]] = arith.constant {ttg.partition = array<i32: 1>} 1 : i32
        // CHECK: [[PSH:%.*]] = arith.shli [[PC1]], [[FSTAGE]] {ttg.partition = array<i32: 1>} : i32
        ...
        %token_6 = nvws.semaphore.acquire %empty {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>], 2> -> !ttg.async.token
        %buffers_5 = nvws.semaphore.buffer %empty, %token_6 {ttg.partition = array<i32: 1>} : !nvws.semaphore<[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>], 2>, !ttg.async.token -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 2x128x128>
        // CHECK: nvws.semaphore.buffer [[EMPTY]][[[FSTAGE]]], [[PTOK]] {ttg.partition = array<i32: 1>}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func public @attention_forward(%arg0: !ttg.memdesc<256x64xf16, #shared, #smem>, %arg1: !tt.tensordesc<tensor<64x64xf16, #shared>>, %arg2: !tt.tensordesc<tensor<64x64xf16, #shared>>, %arg3: f32, %arg4: i32) {
    ...
    // CHECK-DAG: [[E0:%.*]] = nvws.semaphore.create [[TMEM0:%.*]] true {nvws.use_single_phase = true}
    // CHECK-DAG: [[F0:%.*]] = nvws.semaphore.create [[TMEM0]] false {nvws.use_single_phase = true}
    // CHECK-DAG: [[E1:%.*]] = nvws.semaphore.create [[TMEM1:%.*]] true {nvws.use_single_phase = true}
    // CHECK-DAG: [[F1:%.*]] = nvws.semaphore.create [[TMEM1]] false {nvws.use_single_phase = true}
    // CHECK-DAG: [[E2:%.*]] = nvws.semaphore.create [[TMEM2:%.*]] true {nvws.use_single_phase = true}
    // CHECK-DAG: [[F2:%.*]] = nvws.semaphore.create [[TMEM2]] false {nvws.use_single_phase = true}
    ...
    %5:4 = scf.for %arg5 = %c0_i32 to %arg4 step %c64_i32 iter_args(%arg6 = %cst, %arg7 = %cst_1, %arg8 = %token, %arg9 = %token_4) -> (tensor<256xf32, #ttg.slice<{dim = 1, parent = #blocked}>>, tensor<256xf32, #ttg.slice<{dim = 1, parent = #blocked}>>, !ttg.async.token, !ttg.async.token)  : i32 {
      ...
      // CHECK: [[S1_PF_EQ:%.*]] = arith.cmpi eq, [[S1_STAGE]], [[S1_PF_CMP]] {ttg.partition = array<i32: 0, 1>} : i32
      ...
      // CHECK: [[S1_PE_EQ:%.*]] = arith.cmpi eq, [[S1_STAGE]], [[S1_PE_CMP]] {ttg.partition = array<i32: 3>} : i32
    ...
    // CHECK: [[POST_S1E_FLIP:%.*]] = arith.xori [[S1_PF_INIT]], [[POST_S1E_C1]] : i32
    // CHECK: [[POST_S1E_CMP:%.*]] = arith.constant 0 : i32
    // CHECK: [[POST_S1E_EQ:%.*]] = arith.cmpi eq, [[LOOP]]#7, [[POST_S1E_CMP]] : i32
    // CHECK: [[POST_S1E_NEW:%.*]] = arith.select [[POST_S1E_EQ]], [[POST_S1E_FLIP]], [[S1_PF_INIT]] : i32
    // CHECK: [[POST_S1F_C1:%.*]] = arith.constant 1 : i32
    // CHECK: [[POST_S1F_FLIP:%.*]] = arith.xori [[LOOP]]#9, [[POST_S1F_C1]] : i32
    // CHECK: [[POST_S1F_CMP:%.*]] = arith.constant 0 : i32
    // CHECK: [[POST_S1F_EQ:%.*]] = arith.cmpi eq, [[LOOP]]#7, [[POST_S1F_CMP]] : i32
    // CHECK: [[POST_S1F_NEW:%.*]] = arith.select [[POST_S1F_EQ]], [[POST_S1F_FLIP]], [[LOOP]]#9 : i32
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func public @matmul_tma_persistent_ws_kernel(%arg0: !tt.ptr<f8E4M3FN> {tt.divisibility = 16 : i32}, %arg1: !tt.ptr<f8E4M3FN> {tt.divisibility = 16 : i32}, %arg2: !tt.ptr<f8E4M3FN> {tt.divisibility = 16 : i32}, %arg3: i32 {tt.divisibility = 16 : i32}, %arg4: i32 {tt.divisibility = 16 : i32}, %arg5: i32 {tt.divisibility = 16 : i32}, %arg6: i32 {tt.divisibility = 16 : i32}, %arg7: i32 {tt.divisibility = 16 : i32}, %arg8: i32 {tt.divisibility = 16 : i32}) attributes {noinline = false} {
    ...
    %result = ttng.tmem_alloc : () -> !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>
    // CHECK-DAG: [[AB_EMPTY:%.*]] = nvws.semaphore.create [[AB_BUF0:%.*]], [[AB_BUF1:%.*]] true {nvws.use_single_phase = true}
    // CHECK-DAG: [[AB_FULL:%.*]] = nvws.semaphore.create [[AB_BUF2:%.*]], [[AB_BUF3:%.*]] false {nvws.use_single_phase = true}
    // CHECK-DAG: [[ACC_EMPTY:%.*]] = nvws.semaphore.create [[ACC_BUF0:%.*]] true {nvws.use_single_phase = false}
    // CHECK-DAG: [[ACC_FULL:%.*]] = nvws.semaphore.create [[ACC_BUF1:%.*]] false {nvws.use_single_phase = false}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @for_loop_control_operand_ppg(%lb: i32, %ub: i32, %step: i32, %ptr0: !tt.ptr<i32>) {
    ...
    %empty = nvws.semaphore.create %semBuf true : !nvws.semaphore<[!ttg.memdesc<1x128x128xf32, #tmem, #ttng.tensor_memory, mutable>], 1>
    %full = nvws.semaphore.create %semBuf false : !nvws.semaphore<[!ttg.memdesc<1x128x128xf32, #tmem, #ttng.tensor_memory, mutable>], 1>
    ...
    // CHECK: [[FOR0:%.*]]:4 = scf.for {{.*}} iter_args([[F0TOK:%.*]] = [[TOK0]], [[F0STAGE:%.*]] = [[S0]], [[F0PE:%.*]] = [[PF1_OUT]], [[F0PF:%.*]] = [[PF_INIT]]) -> (!ttg.async.token, i32, i32, i32)
    %tok0 = scf.for %iv0 = %lb to %ub step %step iter_args(%tok1 = %tok) -> (!ttg.async.token) : i32 {
      ...
      // CHECK: [[FOR1:%.*]]:2 = scf.for {{.*}} iter_args([[ITOK:%.*]] = [[F0TOK]], [[ISTAGE:%.*]] = [[F0STAGE]]) -> (!ttg.async.token, i32)
      %tok5 = scf.for %iv = %lb1 to %ub1 step %step1 iter_args(%tok2 = %tok1) -> (!ttg.async.token)  : i32 {
        ...
        // CHECK: [[BUF_INNER:%.*]] = nvws.semaphore.buffer [[EMPTY]][[[ISTAGE]]], [[ITOK]] {ttg.partition = array<i32: 2>}
      ...
      // CHECK: [[GC1_EQ:%.*]] = arith.cmpi eq, [[FOR1]]#1, [[GC1_CMP]] {ttg.partition = array<i32: 0, 1>} : i32
      ...
      // CHECK: [[PC1_EQ:%.*]] = arith.cmpi eq, [[FOR1]]#1, [[PC1_CMP]] {ttg.partition = array<i32: 2>} : i32
    ...
    // CHECK: [[POST_FLIPA:%.*]] = arith.xori [[PF_INIT]], [[POST_C1A]] : i32
    // CHECK: [[POST_CMPA:%.*]] = arith.constant 0 : i32
    // CHECK: [[POST_EQA:%.*]] = arith.cmpi eq, [[FOR0]]#1, [[POST_CMPA]] : i32
    // CHECK: arith.select [[POST_EQA]], [[POST_FLIPA]], [[PF_INIT]] : i32
    // CHECK: [[POST_C1B:%.*]] = arith.constant 1 : i32
    // CHECK: [[POST_FLIPB:%.*]] = arith.xori [[FOR0]]#3, [[POST_C1B]] : i32
    // CHECK: [[POST_CMPB:%.*]] = arith.constant 0 : i32
    // CHECK: [[POST_EQB:%.*]] = arith.cmpi eq, [[FOR0]]#1, [[POST_CMPB]] : i32
    // CHECK: [[P_END:%.*]] = arith.select [[POST_EQB]], [[POST_FLIPB]], [[FOR0]]#3 : i32
