...
// CHECK-LABEL:   tt.func @nested_ws_inner_loop(
  tt.func @nested_ws_inner_loop(%lb: i32, %ub: i32, %step: i32) {
    ...
    // ping-pong semaphores are created at the root: EMPTY (true) gates the
    // writer, FULL (false) the reader.
  ...
  tt.func @nested_ws_inner_loop_parent_continuation(%lb: i32, %ub: i32, %step: i32) {
    ...
    // CHECK:           [[LOCAL_EMPTY:%.*]] = nvws.semaphore.create [[LIVE_ALLOC]] true {pending_count = 1 : i32} : <[!ttg.memdesc<1x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>
    // CHECK:           [[OUTER_EMPTY:%.*]] = nvws.semaphore.create [[LIVE_ALLOC]] true {pending_count = 1 : i32} : <[!ttg.memdesc<1x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>
    // CHECK:           [[LOCAL_FULL:%.*]] = nvws.semaphore.create [[LIVE_ALLOC]] false {pending_count = 1 : i32} : <[!ttg.memdesc<1x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>
    // CHECK:           [[OUTER_FULL:%.*]] = nvws.semaphore.create [[LIVE_ALLOC]] false {pending_count = 1 : i32} : <[!ttg.memdesc<1x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>
