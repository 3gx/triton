...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @raw_edge_token_carried_if(%lb: i32, %ub: i32, %step: i32) {
    ...
    %r = scf.for %iv = %lb to %ub step %step iter_args(%i = %c0) -> (i32) : i32 {
      ...
      // CHECK-NEXT: [[B:%.*]] = ttng.tmem_alloc {buffer.id = 401 : i32, buffer.offset = 256 : i32}
      ...
      %b, %tb = ttng.tmem_alloc {buffer.id = 401 : i32, buffer.offset = 256 : i32, ttg.partition = array<i32: 0>} : () -> (!ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.async.token)
