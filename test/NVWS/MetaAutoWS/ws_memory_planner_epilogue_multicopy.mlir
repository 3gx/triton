// RUN: triton-opt %s -split-input-file --nvws-memory-planner="num-buffers=3 smem-alloc-algo=1 smem-budget=220000" | FileCheck %s --check-prefix=LARGE
// RUN: triton-opt %s -split-input-file --nvws-memory-planner="num-buffers=3 smem-alloc-algo=1 smem-budget=200000" | FileCheck %s --check-prefix=TIGHT
...
// LARGE: ttng.tmem_alloc {{.*}}buffer.copy = 2 : i32, buffer.id = {{[0-9]+}} : i32, buffer.offset = 0 : i32
// LARGE: ttng.tmem_load {{.*}}tmem.end

// TIGHT-LABEL: @epilogue_multicopy
// TIGHT: ttng.tmem_alloc {{.*}}buffer.copy = 2 : i32, buffer.id = {{[0-9]+}} : i32, buffer.offset = 0 : i32
// TIGHT: ttng.tmem_load {{.*}}tmem.end
