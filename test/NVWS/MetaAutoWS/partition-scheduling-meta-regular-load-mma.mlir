...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  // CHECK: %[[B:.*]] = tt.load {{.*}}ttg.partition = array<i32: [[GEMM:[0-9]+]]>
  // CHECK: %[[B_SMEM:.*]] = ttg.local_alloc %[[B]] {{.*}}ttg.partition = array<i32: [[GEMM]]>
  // CHECK: ttng.tc_gen5_mma {{.*}}%[[B_SMEM]], {{.*}}ttg.partition = array<i32: [[GEMM]]>
  // CHECK: ttng.tc_gen5_mma {{.*}}%[[B_SMEM]], {{.*}}ttg.partition = array<i32: [[GEMM]]>
  tt.func @regular_load_alloc_mma_operand(%a_desc: !tt.tensordesc<tensor<1x64xf16, #shared>>, %b_ptr_init: tensor<64x128x!tt.ptr<f16>, #b_layout> {tt.divisibility = dense<[16, 16]> : tensor<2xi32>, tt.contiguity = dense<[1, 64]> : tensor<2xi32>}) {
    ...
    scf.for %k = %c0_i32 to %k_tiles step %c1_i32 iter_args(%flag = %true, %b_ptr = %b_ptr_init, %tok_arg0 = %tok0, %tok_arg1 = %tok1) -> (i1, tensor<64x128x!tt.ptr<f16>, #b_layout>, !ttg.async.token, !ttg.async.token) : i32 {
