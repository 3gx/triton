...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  // CHECK-LABEL: @tmem_store_blockm64
  // CHECK: ttng.tmem_store {{.*}} : tensor<64x128xf32, {{.*}}> -> !ttg.memdesc<64x128xf32, #{{.*}}, #ttng.tensor_memory, mutable>
  // CHECK: ttng.tc_gen5_mma {{.*}} : !ttg.memdesc<64x64xf16, #shared, #smem>{{.*}}!ttg.memdesc<64x128xf32, #{{.*}}, #ttng.tensor_memory, mutable>
  tt.func @tmem_store_blockm64(%a_desc: !tt.tensordesc<tensor<128x64xf16, #shared>>, %b_desc: !tt.tensordesc<tensor<64x128xf16, #shared>>) {
    ...
    %init_tok = ttng.tmem_store %zero, %c_tmem[%c_tok], %true : tensor<128x128xf32, #acc_layout> -> !ttg.memdesc<128x128xf32, #acc_tmem, #ttng.tensor_memory, mutable>
    %mma_tok = ttng.tc_gen5_mma %a_shared, %b_shared, %c_tmem[%init_tok], %true, %true : !ttg.memdesc<128x64xf16, #shared, #smem>, !ttg.memdesc<64x128xf16, #shared, #smem>, !ttg.memdesc<128x128xf32, #acc_tmem, #ttng.tensor_memory, mutable>
