// RUN: triton-opt %s --nvws-memory-planner=num-buffers=2 --mlir-print-debuginfo --mlir-use-nameloc-as-prefix 2>&1 | FileCheck %s
...
// CHECK: %dq, %dq_0 = ttng.tmem_alloc {{.*}}buffer.id = {{[0-9]+}} : i32, buffer.offset = {{[0-9]+}} : i32
