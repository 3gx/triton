...
module attributes {"ttg.num-warps" = 4 : i32} {
  ...
  tt.func @basic(%lb: i32, %ub: i32, %step: i32) {
    ...
    scf.for %i = %lb to %ub step %step : i32 {
      ...
      // CHECK: nvws.semaphore.buffer [[EMPTY]], [[PTOK]] {ttg.partition = array<i32: 0>}
      ...
      // CHECK: [[GTOK0:%.*]] = nvws.semaphore.acquire [[FULL]] {ttg.partition = array<i32: 1>}
      ...
      // CHECK: nvws.semaphore.release [[EMPTY]], [[GTOK0]] [#nvws.async_op<none>] {ttg.partition = array<i32: 1>}
      ...
      // CHECK: [[GTOK1:%.*]] = nvws.semaphore.acquire [[FULL]] {ttg.partition = array<i32: 2>}
      ...
      // CHECK: nvws.semaphore.release [[EMPTY]], [[GTOK1]] [#nvws.async_op<none>] {ttg.partition = array<i32: 2>}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @tma_preserved(%desc: !tt.tensordesc<tensor<128x64xf16, #shared>>, %idx: i32) {
    ...
    // CHECK: [[EMPTY:%.*]] = nvws.semaphore.create {{.*}} true : <[!ttg.memdesc<3x128x64xf16
    // CHECK: [[FULL:%.*]] = nvws.semaphore.create {{.*}} false : <[!ttg.memdesc<3x128x64xf16
    %aref = nvws.aref.create %buf : <[!ttg.memdesc<1x128x64xf16, #shared, #smem, mutable>]>

    // CHECK: [[TOK0:%.*]] = nvws.semaphore.acquire [[EMPTY]] {loop.cluster = 1 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>}
    // CHECK: [[BUF0:%.*]] = nvws.semaphore.buffer [[EMPTY]], [[TOK0]] {loop.cluster = 1 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>}
    // CHECK: nvws.descriptor_load {{.*}} [[BUF0]] {loop.cluster = 1 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>}
    ...
    // CHECK: nvws.semaphore.release [[FULL]], [[TOK0]] [#nvws.async_op<tma_load>] {loop.cluster = 1 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>}
    ...
    // CHECK: [[TOK1:%.*]] = nvws.semaphore.acquire [[FULL]] {loop.cluster = 0 : i32, loop.stage = 1 : i32, ttg.partition = array<i32: 1>}
    ...
    // CHECK: nvws.semaphore.release [[EMPTY]], [[TOK1]] [#nvws.async_op<none>] {loop.cluster = 0 : i32, loop.stage = 1 : i32, ttg.partition = array<i32: 1>}
...
module attributes {"ttg.num-warps" = 4 : i32} {
  ...
  tt.func @two_consumers(%arg0: i32, %arg1: i32, %arg2: i32) {
    ...
    // CHECK: [[EMPTY:%.*]] = nvws.semaphore.create {{.*}} true : <[!ttg.memdesc<3x1xi32
    // CHECK: [[FULL:%.*]] = nvws.semaphore.create {{.*}} false : <[!ttg.memdesc<3x1xi32
    ...
    scf.for %arg3 = %arg0 to %arg1 step %arg2 : i32 {
      ...
      // CHECK: [[PTOK:%.*]] = nvws.semaphore.acquire [[EMPTY]] {loop.cluster = 1 : i32, loop.stage = 3 : i32, ttg.partition = array<i32: 0>}
      ...
      // CHECK: nvws.semaphore.release [[FULL]], [[PTOK]] [#nvws.async_op<none>] {loop.cluster = 1 : i32, loop.stage = 3 : i32, ttg.partition = array<i32: 0>}
      ...
      // CHECK: [[GTOK1:%.*]] = nvws.semaphore.acquire [[FULL]] {loop.cluster = 2 : i32, loop.stage = 3 : i32, ttg.partition = array<i32: 1>}
      ...
      // CHECK: nvws.semaphore.release [[EMPTY]], [[GTOK1]] [#nvws.async_op<none>] {loop.cluster = 2 : i32, loop.stage = 3 : i32, ttg.partition = array<i32: 1>}
      ...
      // CHECK: nvws.semaphore.release [[EMPTY]], {{%.*}} [#nvws.async_op<none>] {loop.cluster = 3 : i32, loop.stage = 4 : i32, ttg.partition = array<i32: 2>}
  ...
  tt.func @three_consumers(%arg0: i32, %arg1: i32, %arg2: i32) {
    ...
    scf.for %arg3 = %arg0 to %arg1 step %arg2 : i32 {
      %3 = "op_a"() {ttg.partition = array<i32: 0>} : () -> tensor<1xi32, #blocked>
      // CHECK: nvws.semaphore.acquire [[EMPTY]] {ttg.partition = array<i32: 0>}
      ...
      // CHECK: nvws.semaphore.acquire [[FULL]] {ttg.partition = array<i32: 3>}
  ...
  tt.func @reuse_argument(%arg0: i32, %arg1: i32, %arg2: i32) {
    ...
    scf.for %arg3 = %arg0 to %arg1 step %arg2 iter_args(%arg5 = %cst) -> (tensor<1xi32, #blocked>)  : i32 {
      ...
      // CHECK: nvws.semaphore.acquire [[FULL]] {ttg.partition = array<i32: 2>}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @warp_specialize_tma_matmul(%arg0: i32, %arg1: i32, %arg2: i32, %arg3: !tt.tensordesc<tensor<128x64xf16, #shared>>, %arg4: !tt.tensordesc<tensor<128x64xf16, #shared>>) {
    ...
    %10 = scf.for %arg5 = %c0_i32 to %arg0 step %c1_i32 iter_args(%arg6 = %2) -> (!ttg.async.token)  : i32 {
      ...
      // CHECK: nvws.descriptor_load {{.*}} {loop.cluster = 1 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>}
      // CHECK: nvws.descriptor_load {{.*}} {loop.cluster = 1 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>}
      ...
      // CHECK: nvws.semaphore.release [[FULL]], {{%.*}} [#nvws.async_op<tma_load>] {loop.cluster = 1 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>}
      // CHECK: nvws.semaphore.release [[EMPTY]], {{%.*}} [#nvws.async_op<tc5mma>] {loop.cluster = 0 : i32, loop.stage = 1 : i32, ttg.partition = array<i32: 1>}
    ...
    // CHECK: } {{{.*}}ttg.partition.outputs = [array<i32: 1>]
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @load_used_as_reg_and_smem(%arg0: !tt.tensordesc<tensor<128x64xf16, #shared>>, %arg1: i32) {
    ...
    scf.for %arg2 = %c0_i32 to %arg1 step %c1_i32  : i32 {
      ...
      // CHECK: nvws.semaphore.acquire [[FULL]] {loop.cluster = 0 : i32, loop.stage = 1 : i32, ttg.partition = array<i32: 0>}
      // CHECK: nvws.semaphore.acquire [[FULL]] {loop.cluster = 0 : i32, loop.stage = 1 : i32, ttg.partition = array<i32: 1>}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @load_used_as_reg_and_smem_same_partition(%arg0: !tt.tensordesc<tensor<128x64xf16, #shared>>, %arg1: i32) {
    ...
    scf.for %arg2 = %c0_i32 to %arg1 step %c1_i32  : i32 {
      ...
      // CHECK: nvws.semaphore.acquire [[FULL]] {loop.cluster = 0 : i32, loop.stage = 1 : i32, ttg.partition = array<i32: 0>}
      // CHECK: nvws.semaphore.buffer [[FULL]], {{%.*}} {loop.cluster = 0 : i32, loop.stage = 1 : i32, ttg.partition = array<i32: 0>}
      "use1"(%2) {loop.cluster = 0 : i32, loop.stage = 1 : i32, ttg.partition = array<i32: 0>} : (tensor<128x64xf16, #blocked>) -> ()
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @aref_not_in_loop(%arg0: i32, %arg1: i32, %arg2: i32, %arg3: !tt.tensordesc<tensor<128x64xf16, #shared>>, %arg4: !tt.tensordesc<tensor<128x64xf16, #shared>>) {
    ...
    // CHECK: nvws.semaphore.release [[FULL]], {{%.*}} [#nvws.async_op<tc5mma>]
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func public @attention_forward(%arg0: !ttg.memdesc<256x64xf16, #shared, #smem>, %arg1: !tt.tensordesc<tensor<64x64xf16, #shared>>, %arg2: !tt.tensordesc<tensor<64x64xf16, #shared>>, %arg3: f32, %arg4: i32, %arg5: !tt.ptr<f32>) {
    ...
    // CHECK-DAG: nvws.semaphore.create {{.*}} true
    // CHECK-DAG: nvws.semaphore.create {{.*}} false
    // CHECK-DAG: nvws.semaphore.create {{.*}} true
    // CHECK-DAG: nvws.semaphore.create {{.*}} false
    ...
    %14:4 = scf.for %arg6 = %c0_i32 to %arg4 step %c64_i32 iter_args(%arg7 = %cst, %arg8 = %cst_1, %arg9 = %token, %arg10 = %token_4) -> (tensor<256xf32, #ttg.slice<{dim = 1, parent = #blocked}>>, tensor<256xf32, #ttg.slice<{dim = 1, parent = #blocked}>>, !ttg.async.token, !ttg.async.token)  : i32 {
      // CHECK: nvws.descriptor_load {{.*}} {loop.cluster = 4 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>}
      // CHECK: nvws.semaphore.release {{.*}} [#nvws.async_op<tma_load>] {loop.cluster = 4 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>}
      // CHECK: ttng.tc_gen5_mma
    ...
    // CHECK: } {{{.*}}ttg.partition.outputs = [array<i32: 0>, array<i32: 0>, array<i32: 1>, array<i32: 3>]
