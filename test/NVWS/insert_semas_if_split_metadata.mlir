...
// Regression for the protocol's partition metadata inside scf.if with
// NON-default partitions (mirrors upstream 9860c26c's test): every
// attribute must be derived - nothing may assume partitions 0/1. The pass
// emits the whole protocol inside the ORIGINAL if (no if-split); dead
// token slots are dropped from the if and the loop. See fd75df910d.
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @if_split_workaround_nondefault_partitions(%arg0: !tt.tensordesc<tensor<1x64xf16, #shared>>, %arg1: tensor<64x128x!tt.ptr<f16>, #blocked3> {tt.contiguity = dense<[1, 64]> : tensor<2xi32>, tt.divisibility = dense<16> : tensor<2xi32>}) {
    ...
    %1:3 = scf.for %arg2 = %c0_i32 to %c32_i32 step %c1_i32 iter_args(%arg3 = %true, %arg4 = %arg1, %arg5 = %0) -> (i1, tensor<64x128x!tt.ptr<f16>, #blocked3>, !ttg.async.token)  : i32 {
      ...
      // EMPTY token (the pass renumbers loop.cluster: 2 -> 5).
      ...
      // The protocol nests in the ORIGINAL if (no split). Then-branch:
      // release the MMA round on FULL (partition 4, MMA-tied), consume it
      // (acquire+buffer feed the load, partition 0), hand the slot back on
      // EMPTY, then re-acquire EMPTY for the next round (partition 4) and
      // yield that token. Else-branch yields the carried token unchanged.
      // CHECK: [[IFTOK:%.*]] = scf.if
      ...
      // CHECK: [[NEXTTOK:%.*]] = nvws.semaphore.acquire [[EMPTY]] {loop.cluster = 5 : i32, loop.stage = 2 : i32, ttg.partition = array<i32: 4>} : <[!ttg.memdesc<1x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]> -> !ttg.async.token
      // CHECK: scf.yield {ttg.partition = array<i32: 0, 4>} [[NEXTTOK]] : !ttg.async.token
      // CHECK: } else {
      ...
      // The loop yields the if's token result back into the carry.
      // CHECK: scf.yield {ttg.partition = array<i32: 0, 4, 5>} %{{[-A-Za-z0-9_.$#]+}}, %{{[-A-Za-z0-9_.$#]+}}, [[IFTOK]] : i1, tensor<64x128x!tt.ptr<f16>, #blocked>, !ttg.async.token
  ...
  // REGRESSION (1fd82a2814): the if's partition metadata must route
  // yield-consumed survivors through the loop's AUTHORED
  // partition.outputs entry - NOT the yield op's {0,1,2} union
  // annotation. The bug stamped the if {0,1,2}; partition 2's
  ...
  tt.func @if_split_yield_routing_three_partitions(%lhs: !ttg.memdesc<128x64xf16, #shared, #smem>, %rhs: !ttg.memdesc<64x128xf16, #shared, #smem>) {
    ...
    // The dead token iter_arg is dropped: the loop carries only (i1, i32).
    // CHECK: scf.for %{{[-A-Za-z0-9_.$#]+}} = %{{[-A-Za-z0-9_.$#]+}} to %{{[-A-Za-z0-9_.$#]+}} step %{{[-A-Za-z0-9_.$#]+}} iter_args(%{{[-A-Za-z0-9_.$#]+}} = %{{[-A-Za-z0-9_.$#]+}}, %{{[-A-Za-z0-9_.$#]+}} = %{{[-A-Za-z0-9_.$#]+}}) -> (i1, i32)
    %loop:3 = scf.for %iv = %c0_i32 to %c32_i32 step %c1_i32 iter_args(%use_acc = %false, %tok = %acc_tok, %carry = %c0_i32) -> (i1, !ttg.async.token, i32) : i32 {
      ...
      // The if must NOT pick up partition 2: its partition set and outputs
      // derive from the loop's AUTHORED outputs ({0} and {0,1}), and the
      // {0,1,2} annotation on the loop yield is an over-approximation
      // (regression: an {0,1,2} if leaves partition 2's clone an empty
      // husk using a condition that clone does not have). The protocol
      // nests in the ORIGINAL if; the dead token result is dropped, so the
      // if yields only (i32, i1). Then-branch: release FULL (partition 1,
      // MMA-tied), consume it (acquire+buffer feed the load, partition 0),
      // then release EMPTY.
      // CHECK: [[EPI:%.*]]:2 = scf.if %{{[-A-Za-z0-9_.$#]+}} -> (i32, i1)
      ...
      // Else-branch: hand the round straight back on EMPTY, still MMA-tied.
      // CHECK: } else {
      // CHECK: nvws.semaphore.release [[EMPTY2]], [[BODYTOK]] [#nvws.async_op<tc5mma>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>, !ttg.async.token
      ...
      // The dead token slot is dropped from the loop yield as well.
      // CHECK: scf.yield {ttg.partition = array<i32: 0, 1, 2>} [[EPI]]#1, %{{[-A-Za-z0-9_.$#]+}} : i1, i32
