...
4. Pick the result for the event's `memberIdx`.

5. Rebuild the recorded `viewChain` from that semaphore-buffer result.

6. Retarget the terminal op operand to the rebuilt view or direct buffer.
...
### Loop And If Handling

Loop and branch handling must be based on the access DAG, not token slots.

For `scf.for`:

- determine body entry owner from first access
- determine body exit owner from last access
- insert reconciliation at the loop body boundary when the backedge needs a
  different owner for the next iteration
...
- Fresh old-vs-new equivalence for `064` still passes, including existing 1x/2x
  TMEM semaphore backing-buffer behavior.
- Fresh old-vs-new subpipeline equivalence for `062` still passes, including
  existing 1x/2x TMEM semaphore backing-buffer behavior.
